...
- **A declined card can never seal the shopper inside the payment window.** The XPay SDK stops honoring close requests the moment a payment is submitted and only re-enables them on success, so any failed attempt (a declined card, an abandoned 3DS challenge, a network error) left the window stuck open: its own X did nothing, and neither did Escape or clicking outside. The pay page now listens for the close request the SDK drops and honors it, returning the shopper to the pay page with Pay now rebuilding a fresh window. The window is never closed on a timer or on the decline itself: the decline message stays up for as long as the shopper wants it, and closing happens only when they ask for it. Because that path is only reached after an attempt that did not succeed, the page says so ("Payment was not completed. Your order is saved, and you can try again.") rather than reusing the wording for closing an untouched window. Deliberately scoped to the window's own close button, which the payment page disables while a payment is processing: that makes an arriving close request proof the shopper is not mid-payment, so a live 3DS is never torn down. Reproduced and verified in a browser harness that models the SDK's exact close semantics. Reported to the platform as xpayeg/woocommerce#1.
- **Checkout button says "Pay now", and sessions carry an integration marker.** When an XPay row is selected, the Place Order button reads **Pay now** on both classic and Blocks checkout (order_button_text + placeOrderButtonLabel, sharing the pay page's translated string) — the button says what happens next. Every checkout session's metadata now carries `integration: woocommerce`: visible to support in the Workbench today, and the ready-made data source for a merchant-facing "integration" badge the moment the platform reads it.
- **Eight product decisions, decided and shipped.** A one-by-one design review with the founder settled the remaining behavior questions. (1) A session that expires unpaid now marks the order **failed**, not cancelled: a failed order stays payable, so the emailed pay link keeps working and the pay page's "pay when you are ready" promise is finally true — the expiry note also quotes the decline history embedded in the expired session (how many attempts, last decline reason). (2) Declined attempts are visible as they happen: the plugin subscribes to `payment_intent.payment_failed` and writes an order note per decline (failure code and message), never a status change. (3) Refunds issued from the XPay dashboard now mirror into WooCommerce automatically via `charge.refunded` — recorded (money movement off) and deduplicated against the plugin's own refunds by refund id, using the per-refund presentment mirror for non-EGP orders and an explanatory note when no honest amount exists; `refund.failed` leaves a note too. This also closes the loop for a plugin refund whose response was lost after it committed. (4) A payment landing on a session the order has since replaced is no longer dropped anonymously: the order tracks its own superseded session ids, and money on one of them parks the order **on-hold** with a review note (a second payment on an already-paid order is noted loudly for a dashboard refund). (5) Session replacement stays create-and-expire rather than PATCH-in-place until the platform can reject amount changes while a shopper is mid-payment. (6) The pay-page receipt keeps its full detail for anyone holding the order-key link, matching WooCommerce core's own disclosure on that page. (7) A currency the platform supports but this merchant's account lacks a rate for (exchange_rate_not_found) now raises a standing admin notice naming the currency, cleared by the next successful session or a settings save. (8) The log table's schema check also runs from the daily cron and after updates, so headless stores converge without anyone opening wp-admin. Webhook endpoints now subscribe to five events; the setup dialogs and guides name all five.
