...
- **A declined card can never seal the shopper inside the payment window.** The XPay SDK stops honoring close requests the moment a payment is submitted and only re-enables them on success, so any failed attempt (a declined card, an abandoned 3DS challenge, a network error) left the window stuck open: its own X did nothing, and neither did Escape or clicking outside. The pay page now listens for the close request the SDK drops and honors it, returning the shopper to the same calm "your order is saved, pay when you are ready" state a normal close produces, with Pay now rebuilding a fresh window. Deliberately scoped to the window's own close button, which the payment page disables while a payment is processing: that makes an arriving close request proof the shopper is not mid-payment, so a live 3DS is never torn down. Reproduced and verified in a browser harness that models the SDK's exact close semantics. Reported to the platform as xpayeg/woocommerce#1.
