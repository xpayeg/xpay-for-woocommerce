...
class OrderSessionEndpointTest extends XPayEG_Integration_Test_Case {
	...
	private function call( array $post ): ?array {
		$_POST                     = $post;
		$_POST['nonce']            = wp_create_nonce( 'woocommerce_xpayeg_checkout_elements' );
		...
		ob_start();
		try {
			XPayEG_Checkout_Elements::handle_order_session();
	...
	public function test_a_valid_key_gets_the_orders_session_secret(): void {
		...
		$this->assertSame( 'cs_ops_1', (string) $fresh->get_meta( XPayEG_Constants::META_SESSION_ID ), 'The one-session-per-order ledger must record what was handed out.' );
	}
