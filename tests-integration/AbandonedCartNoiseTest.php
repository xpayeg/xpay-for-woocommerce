...
class AbandonedCartNoiseTest extends XPayEG_Integration_Test_Case {
	...
	private function expired_session( string $payment_status = XPayEG_Payment_Status::UNPAID ): array {
		return array(
			'id'            => 'cs_test_abandoned',
			'status'        => XPayEG_Session_Status::EXPIRED,
			'paymentStatus' => $payment_status,
		);
	}

	public function test_an_abandoned_cart_is_acknowledged_and_not_an_error(): void {
		$this->apply( XPayEG_Event_Names::CHECKOUT_SESSION_EXPIRED, $this->expired_session() );

		$this->assertNotContains(
			'webhook.order_not_found',
			$this->stages_at( XPayEG_Logger::LEVEL_ERROR ),
	...
	public function test_a_completed_session_with_no_order_is_an_error_and_fails(): void {
		...
		$this->assertContains( 'webhook.order_not_found', $this->stages_at( XPayEG_Logger::LEVEL_ERROR ) );
	}

	public function test_a_paid_expiry_with_no_order_is_never_the_quiet_path(): void {
		try {
			$this->apply( XPayEG_Event_Names::CHECKOUT_SESSION_EXPIRED, $this->expired_session( XPayEG_Payment_Status::PAID ) );
	...
	public function test_an_expiry_that_omits_payment_status_is_never_the_quiet_path(): void {
		...
		try {
			$this->apply( XPayEG_Event_Names::CHECKOUT_SESSION_EXPIRED, $session );
