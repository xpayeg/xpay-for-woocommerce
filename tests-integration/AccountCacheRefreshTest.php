...
class AccountCacheRefreshTest extends XPayEG_Integration_Test_Case {
	...
	public function tear_down(): void {
		...
		// leftovers.
		wp_dequeue_script( XPayEG_Checkout_Elements::HANDLE );
		wp_dequeue_script( XPayEG_Checkout_Elements::DRIVER_HANDLE );
		wp_dequeue_style( XPayEG_Checkout_Elements::STYLE_HANDLE );
		delete_option( XPayEG_Constants::account_methods_option( false ) );
		delete_option( XPayEG_Constants::account_checked_option( false ) );
		$GLOBALS['xpayeg_test_http']          = array();
		$GLOBALS['xpayeg_test_http_requests'] = array();
		parent::tear_down();
	}
	...
	private function account_requests(): array {
		$hits = array();
		foreach ( (array) $GLOBALS['xpayeg_test_http_requests'] as $request ) {
	...
	public function test_a_stale_cache_is_reread_once_by_the_checkout_render(): void {
		...
		XPayEG_Checkout_Elements::enqueue();
	...
	public function test_a_fresh_cache_is_not_reread(): void {
		$this->script_account();
		update_option( XPayEG_Constants::account_checked_option( false ), time() - HOUR_IN_SECONDS );
	...
	public function test_a_failed_reread_keeps_the_cache_and_does_not_retry_per_view(): void {
		update_option( XPayEG_Constants::account_checked_option( false ), time() - 3 * HOUR_IN_SECONDS );
		$GLOBALS['xpayeg_test_http'] = array(
			'/account' => array(
