...
class PaymentMethodsTabTest extends XPayEG_Integration_Test_Case {
	...
	public function test_an_unchecked_method_loses_its_row_everywhere(): void {
		update_option( XPayEG_Constants::OPTION_ENABLED_METHODS, array( 'card', 'fawry' ) );

		$this->assertSame( array( 'xpayeg', 'xpayeg_fawry' ), $this->registered_row_ids(), 'An unchecked method must not even register.' );
		$this->assertFalse( $this->gateway()->method_active_for_currency( 'valu' ) );
		$this->assertFalse( ( new XPayEG_Method_Gateway( 'valu' ) )->is_available() );
		$this->assertTrue( ( new XPayEG_Method_Gateway( 'fawry' ) )->is_available() );
	}

	public function test_unchecking_card_hides_the_card_row_but_not_the_others(): void {
		update_option( XPayEG_Constants::OPTION_ENABLED_METHODS, array( 'valu', 'fawry' ) );
	...
	public function test_the_accepted_list_is_the_checked_list_cut_to_the_currency(): void {
		update_option( XPayEG_Constants::OPTION_ENABLED_METHODS, array( 'card', 'valu' ) );
	...
	public function test_a_stored_list_that_lost_every_account_method_offers_nothing(): void {
		...
		update_option( XPayEG_Constants::OPTION_ENABLED_METHODS, array( 'meeza' ) );
		...
		$this->assertSame( array( 'xpayeg' ), $this->registered_row_ids(), 'Only the settings-anchor gateway registers, and it is hidden.' );
	...
	public function test_the_saved_order_arranges_rows_and_registration(): void {
		update_option( XPayEG_Constants::OPTION_METHOD_ORDER, array( 'valu', 'fawry', 'card' ) );

		$this->assertSame( array( 'valu', 'fawry', 'card' ), $this->gateway()->ordered_method_types() );
		$this->assertSame( array( 'xpayeg', 'xpayeg_valu', 'xpayeg_fawry' ), $this->registered_row_ids(), 'The main gateway registers first regardless; positions come from the gateway-order option.' );
	...
	public function test_the_order_heals_against_the_account_map(): void {
		...
		update_option( XPayEG_Constants::OPTION_METHOD_ORDER, array( 'valu', 'meeza', 'card' ) );

		$this->assertSame( array( 'valu', 'card', 'fawry' ), $this->gateway()->ordered_method_types() );
		$this->assertSame(
			array( 'valu', 'meeza', 'card' ),
			get_option( XPayEG_Constants::OPTION_METHOD_ORDER ),
	...
	public function test_sync_inserts_the_rows_at_the_xpayeg_position(): void {
		update_option( XPayEG_Constants::OPTION_METHOD_ORDER, array( 'valu', 'card', 'fawry' ) );
		update_option(
	...
	public function test_sync_appends_when_xpayeg_was_never_ordered(): void {
		update_option( 'woocommerce_gateway_order', array( 'cod' => '0' ) );

		XPayEG_Plugin::sync_gateway_order();

		$order = get_option( 'woocommerce_gateway_order' );
		$this->assertSame( array( 'cod', 'xpayeg', 'xpayeg_valu', 'xpayeg_fawry' ), array_keys( $order ) );
	}
	...
	public function test_rendering_the_settings_screen_heals_a_stale_gateway_order(): void {
		...
		remove_action( 'update_option_woocommerce_gateway_order', array( 'XPayEG_Plugin', 'sync_gateway_order' ) );
		update_option(
			'woocommerce_gateway_order',
			array(
				'xpayeg'       => '0',
		...
		add_action( 'update_option_woocommerce_gateway_order', array( 'XPayEG_Plugin', 'sync_gateway_order' ) );

		ob_start();
		( new XPayEG_Gateway() )->admin_options();
		ob_end_clean();

		$this->assertSame(
			array( 'xpayeg', 'xpayeg_valu', 'xpayeg_fawry', 'cod' ),
			array_keys( get_option( 'woocommerce_gateway_order' ) ),
			'The rows must follow the account order and the relic xpayeg_card entry must be gone.'
		);
	}

	public function test_a_payments_table_save_cannot_erase_the_hidden_rows(): void {
		update_option( 'woocommerce_gateway_order', array( 'xpayeg' => '0', 'cod' => '1', 'xpayeg_valu' => '2', 'xpayeg_fawry' => '3' ) );
		...
		update_option( 'woocommerce_gateway_order', array( 'cod' => '0', 'xpayeg' => '1' ) );

		$this->assertSame(
			array( 'cod', 'xpayeg', 'xpayeg_valu', 'xpayeg_fawry' ),
			array_keys( get_option( 'woocommerce_gateway_order' ) ),
	...
	public function test_the_form_save_persists_the_checked_list(): void {
		$this->save_settings(
			array(
				...
				'xpayeg_method_enabled'  => array( 'card', 'fawry' ),
			)
		);

		$this->assertSame( array( 'card', 'fawry' ), get_option( XPayEG_Constants::OPTION_ENABLED_METHODS ) );
	}

	public function test_unchecking_everything_is_refused_and_keeps_the_previous_list(): void {
		update_option( XPayEG_Constants::OPTION_ENABLED_METHODS, array( 'card', 'valu' ) );

		$this->save_settings( array( 'xpayeg_methods_present' => '1' ) );

		$this->assertSame( array( 'card', 'valu' ), get_option( XPayEG_Constants::OPTION_ENABLED_METHODS ), 'All-unchecked must be refused, never saved.' );
	}
	...
	public function test_a_save_without_the_tab_leaves_the_list_alone(): void {
		...
		$this->assertSame( array( 'card' ), get_option( XPayEG_Constants::OPTION_ENABLED_METHODS ) );
	}
	...
	public function test_unknown_types_in_the_post_never_reach_storage(): void {
		$this->save_settings(
			array(
				'xpayeg_methods_present' => '1',
				'xpayeg_method_enabled'  => array( 'card', 'stolen_type' ),
			)
		);
