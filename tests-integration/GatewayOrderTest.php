...
class GatewayOrderTest extends XPayEG_Integration_Test_Case {

	public function set_up(): void {
		parent::set_up();
		delete_option( XPayEG_Gateway_Order::OPTION_APPLIED );
	...
	private function a_store_where_xpayeg_is_last(): void {
		update_option(
	...
	public function test_xpayeg_is_first_after_activation(): void {
		$this->a_store_where_xpayeg_is_last();
	...
	public function test_every_other_gateway_keeps_its_relative_order(): void {
		...
		$rest = array_values( array_filter( $this->order(), function ( $id ) {
			return ! XPayEG_Constants::is_xpayeg_gateway( $id );
		} ) );
		$this->assertSame(
	...
	public function test_a_run_with_nothing_to_move_does_not_spend_the_one_shot(): void {
		...
		$this->assertSame(
			'',
			(string) get_option( XPayEG_Gateway_Order::OPTION_APPLIED, '' ),
		...
		update_option( 'woocommerce_gateway_order', array( 'bacs' => 0, 'cheque' => 1, 'cod' => 2, 'xpayeg' => 3 ) );
		XPayEG_Gateway_Order::apply_default();

		$this->assertSame( 'xpayeg', $this->order()[0] );
		$this->assertNotSame( '', (string) get_option( XPayEG_Gateway_Order::OPTION_APPLIED, '' ), 'The run that did the work must spend the shot.' );
	}
	...
	public function test_a_merchant_who_reorders_afterwards_is_not_overruled(): void {
		...
		update_option( 'woocommerce_gateway_order', array( 'cod' => 0, 'xpayeg' => 1, 'bacs' => 2 ) );
	...
	public function test_a_store_with_no_xpayeg_row_is_untouched(): void {
