...
abstract class XPayEG_Integration_Test_Case extends WP_UnitTestCase {
	 ...
	 * @return WC_Order
	 */
	protected function make_xpayeg_order( array $meta = array() ): WC_Order {
		$order = new WC_Order();
	...
	protected function configure_gateway( array $settings ): void {
		$existing = get_option( 'woocommerce_xpayeg_settings', array() );
		$existing = is_array( $existing ) ? $existing : array();
		update_option( 'woocommerce_xpayeg_settings', array_merge( $existing, $settings ) );
	}
