<?php
/**
 * A WC_Logger handler that records every "xpayeg"-sourced entry in memory,
 ...
 * the REAL path: XPayEG_Logger's per-level keep/drop decision and the
 * redactor both ran before anything lands in this array. Capturing the
 * xpayeg_logger_event action instead would observe every call site
 * unconditionally and prove nothing about what the log keeps.
 *
 * @package XPayEG_For_WooCommerce
 */

class XPayEG_Spy_Log_Handler extends WC_Log_Handler {
	 ...
	 * @param string $message   Log message (XPayEG_Logger writes a JSON entry).
	 * @param array  $context   Additional information (source).
	 */
	public function handle( $timestamp, $level, $message, $context ) {
		unset( $timestamp, $message );
		if ( ! isset( $context['source'] ) || 'xpayeg' !== $context['source'] ) {
			return false;
		}
