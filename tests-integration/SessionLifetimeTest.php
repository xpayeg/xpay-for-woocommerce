...
class SessionLifetimeTest extends XPayEG_Integration_Test_Case {
	...
	private function sent_bodies(): array {
		$bodies = array();
		foreach ( (array) ( $GLOBALS['xpayeg_test_http_requests'] ?? array() ) as $request ) {
	...
	public function test_the_unpaid_hold_covers_the_pending_window_only(): void {
		$this->assertSame( 2 * HOUR_IN_SECONDS, XPayEG_Order_Sync::UNPAID_GRACE_SECONDS );
	}
}
