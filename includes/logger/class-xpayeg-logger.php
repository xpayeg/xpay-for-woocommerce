 ...
 * (`xpayeg_logger_event`) unconditionally; this class is the only subscriber,
 * and what is worth keeping is decided per write (see init()).
 *
 * Writes go through WC_Logger (source: "xpayeg"): WooCommerce's log directory
 * is already access-hardened, the merchant gets the native
 * WooCommerce → Status → Logs viewer, and retention is handled by core.
 * Every entry passes through XPayEG_Redactor at write time.
 *
...
final class XPayEG_Logger {
	...
	public static function init(): void {
		add_action( 'xpayeg_logger_event', array( __CLASS__, 'write' ), 10, 4 );
	}
	...
	public static function event( string $stage, array $context = array(), string $message = '' ): void {
		do_action( 'xpayeg_logger_event', $stage, $context, $message, self::LEVEL_INFO );
	}
	...
	public static function error( string $stage, array $context = array(), string $message = '' ): void {
		do_action( 'xpayeg_logger_event', $stage, $context, $message, self::LEVEL_ERROR );
	}
	...
	public static function critical( string $stage, array $context = array(), string $message = '' ): void {
		do_action( 'xpayeg_logger_event', $stage, $context, $message, self::LEVEL_CRITICAL );
	}
	...
	public static function write( $stage, $context = array(), $message = '', $level = self::LEVEL_INFO ): void {
		...
		$redacted_context = XPayEG_Redactor::redact( is_array( $context ) ? $context : array() );
		$scrubbed_message = '' !== $message ? XPayEG_Redactor::scrub_string( (string) $message ) : '';
