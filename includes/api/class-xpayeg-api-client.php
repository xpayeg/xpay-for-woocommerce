 ...
 *   4. Convert every non-2xx into a typed XPayEG_Api_Exception.
 *
...
class XPayEG_Api_Client {
	...
	public function __construct( string $api_key ) {
		if ( '' === $api_key ) {
			throw XPayEG_Api_Exception::not_configured( 'API key' );
		}
	 ...
	 * @throws XPayEG_Api_Exception On any non-2xx, carrying the status.
	 */
	...
	private function request( string $method, string $path, array $payload = array(), ?string $idempotency_key = null, ?int $timeout = null ): array {
		$url = XPayEG_Constants::api_base() . $path;
		...
		if ( is_wp_error( $response ) ) {
			...
			throw XPayEG_Api_Exception::transport( 'Could not reach the XPay API' );
		}
		...
		throw XPayEG_Api_Exception::from_api_response( $error_body, $status );
	}
}
