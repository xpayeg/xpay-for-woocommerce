 ...
 * a stable machine-readable code (XPayEG_Error_Codes) separate from the
 * human message, plus the API's own doc_url when the failure came from XPay.
 *
 * Construction is via static factories only: every throw site produces the
 * same envelope, and the code
...
class XPayEG_Api_Exception extends Exception {

	/** @var string Stable code from XPayEG_Error_Codes (or the XPay API). */
	private $error_code;
	...
	public static function not_configured( string $what ): self {
		return new self( 'XPay gateway is not configured: ' . $what, XPayEG_Error_Codes::GATEWAY_NOT_CONFIGURED );
	}
	...
	public static function transport( string $message, ?Throwable $previous = null ): self {
		return new self( $message, XPayEG_Error_Codes::TRANSPORT_ERROR, 0, '', '', $previous );
	}
	...
	public static function from_api_response( array $error_body, int $http_status ): self {
		...
		$code    = '' !== $code ? $code : XPayEG_Error_Codes::API_ERROR;
	...
	/** Webhook signature/timestamp failures — thrown by XPayEG_Signature. */
	...
	/**
	 * The order total cannot be carried as a line quantity.
	 *
	 * The amount rides as the quantity of a one-minor-unit line, so the
	 * platform's own int4 column on that field is the ceiling. Refusing is
	 * the only safe answer: clamping down would undercharge.
	 */
	public static function amount_above_line_ceiling(): self {
		return new self( 'Order total is above the amount a line quantity can carry', XPayEG_Error_Codes::AMOUNT_ABOVE_LINE_CEILING );
	}
	...
	public static function order_lock_busy(): self {
		return new self( 'Another process is updating this order payment state', XPayEG_Error_Codes::ORDER_LOCK_BUSY, 409 );
	}

	/** No currently enabled payment method can serve this order. */
	public static function payment_methods_unavailable(): self {
		return new self( 'No enabled XPay payment method can serve this order', XPayEG_Error_Codes::PAYMENT_METHODS_UNAVAILABLE, 409 );
	}

	/** Secure state or PKCE material could not be generated. */
	public static function connect_randomness_failed(): self {
		return new self( 'Secure Connect flow material could not be generated', XPayEG_Error_Codes::CONNECT_RANDOMNESS_FAILED );
	}
	...
	public static function refund_rejected(): self {
		return new self( 'XPay did not return a completed refund state', XPayEG_Error_Codes::REFUND_REJECTED );
	}
	...
	public static function refund_pending(): self {
		return new self( 'XPay accepted the refund but it is still processing', XPayEG_Error_Codes::REFUND_PENDING );
	}
	...
	public static function refund_currency_unsupported(): self {
		return new self( 'Partial refunds are processed in EGP; this order is priced in another currency', XPayEG_Error_Codes::REFUND_CURRENCY_UNSUPPORTED );
	}
	...
	public static function refund_result_mismatch(): self {
		return new self( 'XPay completed the refund with a different amount or currency than requested', XPayEG_Error_Codes::REFUND_RESULT_MISMATCH );
	}
}
