...
final class XPayEG_Signature {
	 ...
	 *
	 * @throws XPayEG_Api_Exception On any verification failure.
	 */
	public static function verify( string $header, string $raw_body, string $secret, int $tolerance = self::DEFAULT_TOLERANCE, ?int $now = null ): void {
		if ( '' === $secret ) {
			...
			throw XPayEG_Api_Exception::webhook( XPayEG_Error_Codes::WEBHOOK_NOT_CONFIGURED, 'No webhook signing secret is configured' );
		}
		if ( '' === trim( $header ) ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- constant registry-code message with no request data; render sites escape on output.
			throw XPayEG_Api_Exception::webhook( XPayEG_Error_Codes::WEBHOOK_SIGNATURE_MISSING, 'XPay-Signature header is missing' );
		}
		...
		if ( null === $timestamp || array() === $signatures ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- constant registry-code message with no request data; render sites escape on output.
			throw XPayEG_Api_Exception::webhook( XPayEG_Error_Codes::WEBHOOK_SIGNATURE_INVALID, 'XPay-Signature header is malformed' );
		}

		$now = null !== $now ? $now : time();
		if ( abs( $now - $timestamp ) > $tolerance ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- constant registry-code message with no request data; render sites escape on output.
			throw XPayEG_Api_Exception::webhook( XPayEG_Error_Codes::WEBHOOK_TIMESTAMP_TOLERANCE, 'Webhook timestamp is outside the allowed tolerance' );
		}
