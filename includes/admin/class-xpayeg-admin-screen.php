 ...
 *
 * Everything posts the standard woocommerce_xpayeg_* field names into
...
final class XPayEG_Admin_Screen {

	/** Nonce for the status card's AJAX verbs. */
	const NONCE_ACTION = 'xpayeg-admin-screen';

	/** The AJAX verbs, each registered as wp_ajax_xpayeg_admin_{verb}. */
	...
	public static function register(): void {
		...
		foreach ( self::AJAX_VERBS as $verb ) {
			add_action( 'wp_ajax_xpayeg_admin_' . $verb, array( __CLASS__, 'handle_' . $verb ) );
		}
	...
	public static function render_connect_notices(): void {
		...
		if ( XPayEG_Constants::GATEWAY_ID !== $section ) {
			return;
		}
		foreach ( XPayEG_Connect::take_notices() as $notice ) {
			printf(
	...
	public static function enqueue( string $hook ): void {
		...
		wp_enqueue_style(
			'xpayeg-admin',
			XPAYEG_PLUGIN_URL . 'assets/css/admin.css',
			array(),
			XPayEG_Constants::asset_version( 'assets/css/admin.css' )
		);
		wp_enqueue_script(
			'xpayeg-admin',
			XPAYEG_PLUGIN_URL . 'assets/js/admin.js',
			array(),
			XPayEG_Constants::asset_version( 'assets/js/admin.js' ),
			true
		);
		wp_localize_script(
			'xpayeg-admin',
	 ...
	 *
	 * @param XPayEG_Gateway $gateway The gateway whose settings these are.
	 */
	public static function render( XPayEG_Gateway $gateway ): void {
		$live      = ! $gateway->is_test_mode();
		$connected = self::keys_validated( $gateway, $live );
		$methods   = $gateway->ordered_method_types();

		echo '<div class="xpayeg-ad" data-xpayeg-admin>';
		...
		if ( array() !== $methods ) {
			...
			echo '<div class="xpayeg-ad__page" data-xpayeg-page="methods">';
			self::methods_card( $gateway, $methods );
			echo '</div>';

			echo '<div class="xpayeg-ad__page" data-xpayeg-page="settings" hidden>';
	...
	private static function page_tabs(): void {
		...
		echo '<div class="xpayeg-ad__tabs xpayeg-ad__tabs--page" role="tablist">';
		foreach ( $tabs as $key => $label ) {
			$active = 'methods' === $key;
			echo '<button type="button" class="xpayeg-ad__tab' . ( $active ? ' is-active' : '' ) . '" role="tab" aria-selected="' . ( $active ? 'true' : 'false' ) . '" data-xpayeg-page-tab="' . esc_attr( $key ) . '">' . esc_html( $label ) . '</button>';
		}
		echo '</div>';
	}
	...
	private static function methods_card( XPayEG_Gateway $gateway, array $ordered ): void {
		...
		echo '<h2 class="xpayeg-ad__card-title">' . esc_html__( 'Payment methods', 'xpay-for-woocommerce' ) . '</h2>';
		echo '<span class="xpayeg-ad__spacer"></span>';
		/*
		 ...
		 */
		echo '<span class="xpayeg-ad__methods-idle" data-xpayeg-methods-idle>';
		echo '<button type="button" class="xpayeg-ad__btn xpayeg-ad__btn--tertiary" data-xpayeg-reorder-start>' . esc_html__( 'Change display order', 'xpay-for-woocommerce' ) . '</button>';
		echo '<div class="xpayeg-ad__menu" data-xpayeg-menu>';
		echo '<button type="button" class="xpayeg-ad__menu-btn" data-xpayeg-menu-toggle aria-haspopup="true" aria-expanded="false" aria-label="' . esc_attr__( 'Payment methods menu', 'xpay-for-woocommerce' ) . '">⋮</button>';
		echo '<div class="xpayeg-ad__menu-list" hidden>';
		...
		// the tab follows.
		echo '<button type="button" class="xpayeg-ad__menu-item" data-xpayeg-refresh-account>' . esc_html__( 'Refresh payment methods', 'xpay-for-woocommerce' ) . '</button>';
		echo '</div></div>';
		echo '</span>';
		echo '<span class="xpayeg-ad__reorder-actions" hidden>';
		echo '<button type="button" class="xpayeg-ad__btn xpayeg-ad__btn--tertiary" data-xpayeg-reorder-cancel>' . esc_html__( 'Cancel', 'xpay-for-woocommerce' ) . '</button>';
		echo '<button type="button" class="xpayeg-ad__btn xpayeg-ad__btn--outline" data-xpayeg-reorder-save>' . esc_html__( 'Save display order', 'xpay-for-woocommerce' ) . '</button>';
		echo '</span>';
		echo '</div>';

		echo '<p class="xpayeg-ad__muted">' . esc_html__( 'Choose which payment methods appear at checkout. Manage available methods in your XPay dashboard.', 'xpay-for-woocommerce' ) . '</p>';
		...
		echo '<input type="hidden" name="xpayeg_methods_present" value="1">';

		echo '<p class="xpayeg-ad__muted xpayeg-ad__methods-note">' . esc_html__( 'This only changes the order of XPay methods. Manage all payment methods in WooCommerce → Settings → Payments.', 'xpay-for-woocommerce' ) . '</p>';

		echo '<div class="xpayeg-ad__actions"><button type="submit" name="save" class="button button-primary" value="save">' . esc_html__( 'Save changes', 'xpay-for-woocommerce' ) . '</button></div>';
	 ...
	 */
	private static function header( XPayEG_Gateway $gateway, bool $live, bool $connected ): void {
		unset( $gateway, $connected );
		echo '<div class="xpayeg-ad__header">';
		echo '<img class="xpayeg-ad__mark" src="' . esc_url( XPAYEG_PLUGIN_URL . 'assets/images/xpay-icon.svg' ) . '" alt="XPay">';
		echo '<span class="xpayeg-ad__name">XPay</span>';
		echo '<span class="xpayeg-ad__pill xpayeg-ad__pill--' . ( $live ? 'live' : 'test' ) . '">' . esc_html( $live ? __( 'Live mode', 'xpay-for-woocommerce' ) : __( 'Test mode', 'xpay-for-woocommerce' ) ) . '</span>';
		echo '<span class="xpayeg-ad__spacer"></span>';
		echo '<a class="xpayeg-ad__ext" href="' . esc_url( XPayEG_Constants::DASHBOARD_URL ) . '" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Open XPay dashboard', 'xpay-for-woocommerce' ) . ' ↗</a>';
		echo '</div>';
	}
	...
	private static function get_started_card(): void {
		echo '<div class="xpayeg-ad__card xpayeg-ad__card--hero">';

		echo '<img class="xpayeg-ad__hero-logo" src="' . esc_url( XPAYEG_PLUGIN_URL . 'assets/images/xpay-mark.svg' ) . '" alt="XPay">';

		echo '<h2 class="xpayeg-ad__hero-title">' . esc_html__( 'Start accepting payments', 'xpay-for-woocommerce' ) . '</h2>';
		echo '<p class="xpayeg-ad__hero-sub">' . esc_html__( 'Connect your XPay account to start accepting payments. Setup is automatic, and card details never touch your server.', 'xpay-for-woocommerce' ) . '</p>';
		 ...
		 */
		echo '<div class="xpayeg-ad__hero-actions">';
		self::connect_button( 'test', __( 'Connect with XPay', 'xpay-for-woocommerce' ), 'button button-primary button-hero' );
		echo '<span class="xpayeg-ad__hero-aside">' . esc_html__( 'Starts in test mode. Nothing goes live until you switch it yourself.', 'xpay-for-woocommerce' ) . '</span>';
		echo '</div>';
		echo '<p class="xpayeg-ad__connect-error" data-xpayeg-connect-error hidden></p>';
		if ( ! XPayEG_Connect::https_ready() ) {
			echo '<p class="xpayeg-ad__auth-help">' . esc_html__( 'Connecting needs your site served over HTTPS.', 'xpay-for-woocommerce' ) . '</p>';
		}
		...
		echo '<div class="xpayeg-ad__hero-methods">';
		foreach ( array( XPayEG_Payment_Methods::CARD, XPayEG_Payment_Methods::VALU, XPayEG_Payment_Methods::FAWRY ) as $type ) {
			$icon = XPayEG_Payment_Methods::icon_url( $type );
			if ( '' === $icon ) {
				continue;
			}
			echo '<span class="xpayeg-ad__hero-method"><img src="' . esc_url( $icon ) . '" alt="' . esc_attr( XPayEG_Payment_Methods::label( $type ) ) . '"></span>';
		}
		echo '</div>';
	...
	private static function connect_button( string $mode, string $label, string $classes ): void {
		if ( XPayEG_Connect::https_ready() ) {
			echo '<button type="button" class="' . esc_attr( $classes ) . '" data-xpayeg-connect data-xpayeg-plane="' . esc_attr( $mode ) . '">' . esc_html( $label ) . '</button>';
			return;
		}
	 ...
	 * @param bool         $connected Active mode's keys are proved.
	 */
	private static function status_card( XPayEG_Gateway $gateway, bool $live, bool $connected ): void {
		$mode            = $live ? 'live' : 'test';
		$endpoint        = XPayEG_Webhook_Configurator::endpoint_data( $live );
		...
		echo '<div class="xpayeg-ad__card-head">';
		echo '<h2 class="xpayeg-ad__card-title">' . esc_html__( 'Account status', 'xpay-for-woocommerce' ) . '</h2>';
		echo '<span class="xpayeg-ad__spacer"></span>';
		if ( '' !== $merchant_name || '' !== $merchant_id ) {
			...
			echo '<span class="xpayeg-ad__identity">';
			if ( '' !== $merchant_name ) {
				echo '<span class="xpayeg-ad__identity-name">' . esc_html( $merchant_name ) . '</span>';
			}
			if ( '' !== $merchant_id ) {
				echo '<span class="xpayeg-ad__muted xpayeg-ad__mono">' . esc_html( $merchant_id ) . '</span>';
			}
			echo '</span>';
		}
		...
		if ( $webhook_ready ) {
			echo '<p class="xpayeg-ad__health" data-xpayeg-health data-xpayeg-plane="' . esc_attr( $mode ) . '">';
			echo '<span data-xpayeg-health-message>' . esc_html( XPayEG_Webhook_State::status_message( $live ) ) . '</span> ';
			echo '<button type="button" class="button-link" data-xpayeg-refresh-health>' . esc_html__( 'Refresh', 'xpay-for-woocommerce' ) . '</button>';
			echo '</p>';
		}
		echo '</div>';
	}
	...
	private static function kebab_menu( bool $live, bool $connected ): void {
		echo '<div class="xpayeg-ad__menu" data-xpayeg-menu>';
		echo '<button type="button" class="xpayeg-ad__menu-btn" data-xpayeg-menu-toggle aria-haspopup="true" aria-expanded="false" aria-label="' . esc_attr__( 'Connection actions', 'xpay-for-woocommerce' ) . '">⋮</button>';
		echo '<div class="xpayeg-ad__menu-list" hidden>';
		echo '<button type="button" class="xpayeg-ad__menu-item" data-xpayeg-open-modal>' . esc_html__( 'Configure connection', 'xpay-for-woocommerce' ) . '</button>';
		echo '<button type="button" class="xpayeg-ad__menu-item" data-xpayeg-refresh-account>' . esc_html__( 'Refresh account details', 'xpay-for-woocommerce' ) . '</button>';
		if ( $connected ) {
			echo '<button type="button" class="xpayeg-ad__menu-item xpayeg-ad__menu-item--danger" data-xpayeg-disconnect data-xpayeg-plane="' . esc_attr( $live ? 'live' : 'test' ) . '">' . esc_html__( 'Disconnect', 'xpay-for-woocommerce' ) . '</button>';
		}
		echo '</div></div>';
	}
	...
	private static function badge( string $label, bool $ok, string $value ): void {
		echo '<span class="xpayeg-ad__badge"><span class="xpayeg-ad__badge-label">' . esc_html( $label ) . '</span><span class="xpayeg-ad__badge-value xpayeg-ad__badge-value--' . ( $ok ? 'ok' : 'warn' ) . '">' . esc_html( $value ) . '</span></span>';
	}
	 ...
	 * @param bool         $live    Live mode selected.
	 */
	private static function settings_card( XPayEG_Gateway $gateway, bool $live ): void {
		echo '<div class="xpayeg-ad__card">';
		echo '<h2 class="xpayeg-ad__card-title">' . esc_html__( 'Settings', 'xpay-for-woocommerce' ) . '</h2>';

		echo '<div class="xpayeg-ad__fields">';
		...
		echo '<label class="xpayeg-ad__field xpayeg-ad__field--check">';
		echo '<input type="checkbox" data-xpayeg-testmode' . checked( $test, true, false ) . disabled( $locked_test || $locked_live, true, false ) . '>';
		echo '<span class="xpayeg-ad__field-main"><span class="xpayeg-ad__field-label">' . esc_html__( 'Enable test mode', 'xpay-for-woocommerce' ) . '</span>';
		echo '<span class="xpayeg-ad__field-help">' . esc_html__( 'Test mode never charges real money.', 'xpay-for-woocommerce' );
		if ( $locked_test ) {
			echo '<strong class="xpayeg-ad__lock-notice">' . esc_html__( 'Live mode cannot be enabled before you have connected a live XPay account.', 'xpay-for-woocommerce' ) . ' ';
			echo '<button type="button" class="button-link" data-xpayeg-open-modal data-xpayeg-modal-tab="live">' . esc_html__( 'Connect your live account', 'xpay-for-woocommerce' ) . '</button></strong>';
		}
		if ( $locked_live ) {
			echo '<strong class="xpayeg-ad__lock-notice">' . esc_html__( 'Test mode cannot be enabled before you have connected a test XPay account.', 'xpay-for-woocommerce' ) . ' ';
			echo '<button type="button" class="button-link" data-xpayeg-open-modal data-xpayeg-modal-tab="test">' . esc_html__( 'Connect your test account', 'xpay-for-woocommerce' ) . '</button></strong>';
		}
		echo '</span></span>';
		echo '</label>';
		echo '<input type="hidden" name="' . esc_attr( self::field_name( $gateway, 'mode' ) ) . '" value="' . esc_attr( $test ? 'test' : 'live' ) . '" data-xpayeg-mode-carrier>';
		...
		echo '<div class="xpayeg-ad__field">';
		echo '<span class="xpayeg-ad__field-label">' . esc_html__( 'Theme', 'xpay-for-woocommerce' ) . '</span>';
		echo '<span class="xpayeg-ad__field-main">';
		$current = (string) $gateway->get_option( 'color_mode', 'auto' );
		echo '<div class="xpayeg-ad__segment" role="radiogroup" aria-label="' . esc_attr__( 'Theme', 'xpay-for-woocommerce' ) . '">';
		foreach ( array(
			'auto'  => __( 'Automatic', 'xpay-for-woocommerce' ),
			'light' => __( 'Light', 'xpay-for-woocommerce' ),
			'dark'  => __( 'Dark', 'xpay-for-woocommerce' ),
		) as $value => $label ) {
			echo '<label class="xpayeg-ad__segment-opt' . ( $current === $value ? ' is-active' : '' ) . '">';
			echo '<input type="radio" name="' . esc_attr( self::field_name( $gateway, 'color_mode' ) ) . '" value="' . esc_attr( $value ) . '"' . checked( $current, $value, false ) . '>';
			echo esc_html( $label );
			echo '</label>';
		}
		echo '</div>';
		echo '<span class="xpayeg-ad__field-help">' . esc_html__( 'Payment fields use the branding from your XPay dashboard. Automatic matches your checkout theme.', 'xpay-for-woocommerce' ) . '</span>';
		echo '</span></div>';

		self::checkbox_field( $gateway, 'debug', __( 'Diagnostic logging', 'xpay-for-woocommerce' ), __( 'Write redacted diagnostic logs to WooCommerce → Status → Logs (source "xpayeg"). Failures are always recorded either way.', 'xpay-for-woocommerce' ) );
		...
		echo '<div class="xpayeg-ad__actions"><button type="submit" name="save" class="button button-primary" value="save">' . esc_html__( 'Save changes', 'xpay-for-woocommerce' ) . '</button></div>';
		echo '<p class="xpayeg-ad__docs">' . wp_kses(
			sprintf(
	 ...
	 * @param bool         $live    Live mode selected.
	 */
	private static function connection_modal( XPayEG_Gateway $gateway, bool $live ): void {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only display routing; nothing changes state.
		$autopen = isset( $_GET['xpayeg-setup'] );
		echo '<div class="xpayeg-ad__overlay" data-xpayeg-modal' . ( $autopen ? ' data-xpayeg-autopen' : '' ) . ' hidden>';
		echo '<div class="xpayeg-ad__modal" role="dialog" aria-modal="true" aria-label="' . esc_attr__( 'XPay account and webhooks', 'xpay-for-woocommerce' ) . '">';

		echo '<div class="xpayeg-ad__modal-head">';
		echo '<h2 class="xpayeg-ad__card-title">' . esc_html__( 'XPay account & webhooks', 'xpay-for-woocommerce' ) . '</h2>';
		echo '<button type="button" class="xpayeg-ad__close" data-xpayeg-close-modal aria-label="' . esc_attr__( 'Close', 'xpay-for-woocommerce' ) . '">×</button>';
		echo '</div>';

		echo '<div class="xpayeg-ad__tabs" role="tablist">';
		foreach ( array( 'test', 'live' ) as $tab ) {
			$selected = ( 'live' === $tab ) === $live;
			echo '<button type="button" class="xpayeg-ad__tab' . ( $selected ? ' is-active' : '' ) . '" role="tab" aria-selected="' . ( $selected ? 'true' : 'false' ) . '" data-xpayeg-tab="' . esc_attr( $tab ) . '">' . esc_html( 'live' === $tab ? __( 'Live', 'xpay-for-woocommerce' ) : __( 'Test', 'xpay-for-woocommerce' ) ) . '</button>';
		}
		echo '</div>';
	 ...
	 */
	private static function modal_pane( XPayEG_Gateway $gateway, bool $pane_live, bool $visible ): void {
		$mode          = $pane_live ? 'live' : 'test';
		$connected     = self::keys_validated( $gateway, $pane_live );
		$endpoint      = XPayEG_Webhook_Configurator::endpoint_data( $pane_live );
		...
		echo '<div class="xpayeg-ad__pane" data-xpayeg-pane="' . esc_attr( $mode ) . '"' . ( $visible ? '' : ' hidden' ) . '>';
		...
		echo '<h3 class="xpayeg-ad__auth-heading">' . esc_html( $pane_live ? __( 'Connect with XPay in live mode', 'xpay-for-woocommerce' ) : __( 'Connect with XPay in test mode', 'xpay-for-woocommerce' ) ) . '</h3>';
		echo '<p class="xpayeg-ad__auth-sub">' . esc_html__( 'Connect your XPay account. Payment methods and order updates are set up automatically.', 'xpay-for-woocommerce' ) . '</p>';

		echo '<div class="xpayeg-ad__auth-actions">';
		self::connect_button(
			$mode,
		...
		if ( $has_key ) {
			...
			echo '<button type="button" class="button" data-xpayeg-reconfigure data-xpayeg-plane="' . esc_attr( $mode ) . '">' . esc_html__( 'Reconfigure webhook', 'xpay-for-woocommerce' ) . '</button>';
		}
		echo '</div>';
		echo '<p class="xpayeg-ad__connect-error" data-xpayeg-connect-error hidden></p>';
		if ( ! XPayEG_Connect::https_ready() ) {
		...
		echo '<p class="xpayeg-ad__auth-help" data-xpayeg-reconfigure-result></p>';

		echo '<p class="xpayeg-ad__auth-help">' . wp_kses(
			sprintf(
				$webhook_ready
				...
				'<strong>' . esc_html( XPayEG_Webhook_Configurator::webhook_url() ) . '</strong>'
			),
			array( 'strong' => array() )
		) . '</p>';
	...
	private static function connect_diagram(): void {
		...
		echo '<div class="xpayeg-ad__diagram" aria-hidden="true">';
		if ( '' !== $site_icon ) {
			echo '<span class="xpayeg-ad__diagram-tile"><img src="' . esc_url( $site_icon ) . '" alt=""></span>';
		} else {
			echo '<span class="xpayeg-ad__diagram-tile xpayeg-ad__diagram-tile--store"><span class="dashicons dashicons-store"></span></span>';
		}
		echo '<span class="xpayeg-ad__diagram-line"></span>';
		echo '<span class="xpayeg-ad__diagram-tile xpayeg-ad__diagram-tile--xpay"><img src="' . esc_url( XPAYEG_PLUGIN_URL . 'assets/images/xpay-icon.svg' ) . '" alt=""></span>';
		echo '</div>';
	}
	 ...
	 */
	private static function checkbox_field( XPayEG_Gateway $gateway, string $key, string $label, string $help ): void {
		echo '<label class="xpayeg-ad__field xpayeg-ad__field--check">';
		...
		echo '<span class="xpayeg-ad__field-main"><span class="xpayeg-ad__field-label">' . esc_html( $label ) . '</span>';
		echo '<span class="xpayeg-ad__field-help">' . esc_html( $help ) . '</span></span>';
		echo '</label>';
	}

	/** @param XPayEG_Gateway $gateway Gateway. @param string $key Settings key. */
	private static function field_name( XPayEG_Gateway $gateway, string $key ): string {
	 ...
	 */
	public static function keys_validated( XPayEG_Gateway $gateway, bool $live ): bool {
		$mode        = $live ? 'live' : 'test';
		...
		$proof = get_option( XPayEG_Constants::OPTION_KEY_VALIDATED, array() );
		...
		return hash_equals( (string) $proof['fingerprint'], XPayEG_Constants::key_fingerprint( $secret, $publishable ) );
	}
	...
	private static function live_payments_disabled(): bool {
		return '1' === get_option( 'xpayeg_live_payments_disabled', '' );
	}
	...
	public static function handle_health(): void {
		...
		wp_send_json_success(
			array(
				'message' => XPayEG_Webhook_State::status_message( $live ),
				'code'    => XPayEG_Webhook_State::status_code( $live ),
			)
		);
	}
	...
	public static function handle_refresh_account(): void {
		self::verify();
		$gateway = XPayEG_Plugin::instance()->gateway();
		try {
	...
	public static function handle_reconfigure_webhooks(): void {
		...
		$rate_key = 'xpayeg-reconfigure-' . ( $live ? 'live' : 'test' ) . '-' . get_current_user_id();
		...
		try {
			XPayEG_Webhook_Configurator::configure( $key );
		} catch ( XPayEG_Api_Exception $e ) {
			wp_send_json_error( array( 'message' => XPayEG_Webhook_State::reason_sentence( $e->get_error_code() ) ), 502 );
		}
	...
	public static function handle_save_method_order(): void {
		...
		update_option( XPayEG_Constants::OPTION_METHOD_ORDER, array_values( $posted ), false );
		XPayEG_Plugin::sync_gateway_order();

		XPayEG_Logger::event( 'admin.method_order_saved', array( 'order' => implode( ',', $posted ) ) );
	...
	public static function handle_connect(): void {
		...
		try {
			$url = XPayEG_Connect::begin( $live );
		} catch ( XPayEG_Api_Exception $e ) {
			XPayEG_Logger::error( 'connect.begin_failed', array( 'code' => $e->get_error_code() ) );
	...
	public static function handle_disconnect(): void {
		...
		XPayEG_Webhook_Configurator::maybe_decommission( XPayEG_Webhook_Configurator::endpoint_data( $live ), '' );
		...
		XPayEG_Webhook_State::clear_state( $live );
