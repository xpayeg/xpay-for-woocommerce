 ...
 * WooCommerce → Status → Logs (source "xpayeg").
 *
...
final class XPayEG_Order_Panel {
	...
	public static function render_refund_currency_note( $order_id ): void {
		...
		if ( XPayEG_Constants::SETTLEMENT_CURRENCY === strtoupper( (string) $order->get_currency() ) ) {
			return;
		}
		...
		echo esc_html(
			sprintf(
				...
				XPayEG_Constants::SETTLEMENT_CURRENCY
			)
		);
		echo '</div></td></tr>';
	}
	...
	public static function render( $post_or_order ): void {
		...
		$attempt    = (int) $order->get_meta( XPayEG_Constants::META_ATTEMPT );

		$customer_id = (string) $order->get_meta( XPayEG_Constants::META_CUSTOMER_ID );
