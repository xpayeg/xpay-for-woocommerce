<?php
/**
 * XPayEG_Webhook_Controller
 *
 * Public webhook receiver at ?wc-api=xpayeg_webhook.
 *
 * Trust boundary: raw internet. The HMAC signature (XPayEG_Signature) is the
...
class XPayEG_Webhook_Controller {
	...
	public static function handle(): void {
		...
		try {
			XPayEG_Signature::verify( $header, $raw_body, $secret );
			...
			if ( XPayEG_Error_Codes::WEBHOOK_NOT_CONFIGURED === $e->get_error_code() ) {
		...
		if ( ! is_array( $event ) || empty( $event['type'] ) || ! is_string( $event['type'] ) || ! isset( $event['data']['object'] ) || ! is_array( $event['data']['object'] ) ) {
			XPayEG_Logger::error( 'webhook.rejected', array( 'code' => XPayEG_Error_Codes::WEBHOOK_PAYLOAD_MALFORMED ) );
			self::record_failure( $gateway, XPayEG_Error_Codes::WEBHOOK_PAYLOAD_MALFORMED );
			self::respond( 400, array( 'error' => XPayEG_Error_Codes::WEBHOOK_PAYLOAD_MALFORMED ) );
		}
		...
		XPayEG_Webhook_State::record_success( ! empty( $event['livemode'] ) );
		...
		try {
			...
			if ( XPayEG_Error_Codes::ORDER_MISMATCH === $e->get_error_code() ) {
				XPayEG_Logger::event( 'webhook.ownership_mismatch', array( 'event_id' => $event_id ) );
				self::respond(
					200,
					array(
						'received' => true,
			...
			self::record_failure( $gateway, XPayEG_Error_Codes::WEBHOOK_APPLY_FAILED );
			self::respond( 500, array( 'error' => 'internal' ) );
		}
	 ...
	 * `curl -X POST '<store>/?wc-api=xpayeg_webhook' -d '{}'` wrote the
	 * failure option the settings screen renders as "Webhook failing"
	 * (class-xpayeg-settings-screen.php:494) AND an ERROR row that survives
	 * diagnostics being off (class-xpayeg-logger.php:103). Anyone who knew
	 ...
	 *
	 * @param XPayEG_Gateway $gateway The gateway, for the configured plane.
	 * @param string       $header  Raw XPay-Signature header as received.
	 * @param string       $code    Error code from the rejection path.
	 */
	private static function record_rejection( XPayEG_Gateway $gateway, string $header, string $code ): void {
		// Same emptiness test the verifier itself uses for a missing header
		// (class-xpayeg-signature.php:43), so the two cannot drift apart.
		if ( '' === trim( $header ) ) {
			XPayEG_Logger::event(
				'webhook.rejected',
				array(
	...
	private static function is_abandoned_cart( string $type, array $payload ): bool {
		if ( XPayEG_Event_Names::CHECKOUT_SESSION_EXPIRED !== $type ) {
			return false;
		}
		return isset( $payload['paymentStatus'] )
			&& XPayEG_Payment_Status::UNPAID === (string) $payload['paymentStatus'];
	}
	 ...
	 * @param array  $payload  data.object payload (a checkout session).
	 * @throws XPayEG_Api_Exception On ownership mismatch, a busy per-order lock, or an order not found yet.
	 */
	private static function apply_event( string $type, string $event_id, array $payload ): void {
		if ( ! in_array( $type, XPayEG_Event_Names::SUBSCRIBED, true ) ) {
		...
		if ( null === $order ) {
			...
			throw XPayEG_Api_Exception::webhook( XPayEG_Error_Codes::WEBHOOK_ORDER_NOT_FOUND, 'No order carries this event yet' );
		}
		...
		if ( in_array( $type, self::SESSION_SCOPED, true ) && 'foreign' === self::session_relation( $order, self::event_session_id( $type, $payload ) ) ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- constant message; the webhook response body carries only the stable code, never this text.
			throw XPayEG_Api_Exception::webhook( XPayEG_Error_Codes::ORDER_MISMATCH, 'Session does not belong to this order' );
		}
		...
		try {
			...
			$order = XPayEG_Order_Sync::reload( $order_id );
			if ( null === $order ) {
				return;
			}
			...
			if ( in_array( $type, self::SESSION_SCOPED, true ) ) {
				...
				if ( 'foreign' === $relation ) {
					XPayEG_Logger::event( 'webhook.relation_changed_in_lock', array( 'event_id' => $event_id ) );
					return;
				}
			}

			switch ( $type ) {
				case XPayEG_Event_Names::CHECKOUT_SESSION_COMPLETED:
					if ( 'current' === $relation ) {
						...
						XPayEG_Order_Sync::apply_completed( $order, $payload, 'webhook' );
					} else {
						...
						// ignored there.
						XPayEG_Order_Sync::apply_superseded_paid( $order, $payload );
					}
					break;
				case XPayEG_Event_Names::CHECKOUT_SESSION_ASYNC_PAYMENT_SUCCEEDED:
					if ( 'current' === $relation ) {
						...
						XPayEG_Order_Sync::mark_paid( $order, $payload, 'webhook' );
					} else {
					...
					break;
				case XPayEG_Event_Names::CHECKOUT_SESSION_ASYNC_PAYMENT_FAILED:
					if ( 'current' === $relation ) {
						XPayEG_Order_Sync::mark_async_failed( $order, $payload );
					} else {
					...
					break;
				case XPayEG_Event_Names::CHECKOUT_SESSION_EXPIRED:
					if ( 'current' === $relation ) {
						XPayEG_Order_Sync::mark_expired( $order, $payload );
					} else {
					...
					break;
				case XPayEG_Event_Names::PAYMENT_INTENT_FAILED:
					// Both 'current' and 'superseded' are this order's own
					// attempts — a decline is order history either way.
					XPayEG_Order_Sync::note_payment_failed( $order, $payload );
					break;
				case XPayEG_Event_Names::CHARGE_REFUNDED:
					XPayEG_Refund_Service::mirror_charge_refunds( $order, $payload );
					break;
				case XPayEG_Event_Names::REFUND_FAILED:
					XPayEG_Refund_Service::note_refund_failed( $order, $payload );
					break;
			}
			...
			XPayEG_Order_Lock::release( $order_id );
		}
	}
	...
	private static function locate_order( string $type, array $payload ): ?WC_Order {
		if ( XPayEG_Event_Names::CHARGE_REFUNDED === $type || XPayEG_Event_Names::REFUND_FAILED === $type ) {
		...
		if ( $order instanceof WC_Order && XPayEG_Constants::is_xpayeg_gateway( (string) $order->get_payment_method() ) ) {
			return $order;
		}
		...
		$found = self::order_id_by_meta( XPayEG_Constants::META_SESSION_ID, $session_id );
		if ( 0 === $found ) {
			return null;
		}
	...
	private static function order_by_payment_intent( array $payload ): ?WC_Order {
		...
		$order_id = self::order_id_by_meta( XPayEG_Constants::META_PAYMENT_INTENT, $intent_id );
		if ( 0 === $order_id ) {
			return null;
		}
		 ...
		 */
		if ( (string) $order->get_meta( XPayEG_Constants::META_PAYMENT_INTENT ) !== $intent_id ) {
			XPayEG_Logger::error(
	...
	private static function event_session_id( string $type, array $payload ): string {
		if ( XPayEG_Event_Names::PAYMENT_INTENT_FAILED === $type ) {
	...
	private static function session_relation( WC_Order $order, string $incoming ): string {
		$stored = trim( (string) $order->get_meta( XPayEG_Constants::META_SESSION_ID ) );
		if ( '' === $stored || '' === $incoming ) {
			return 'foreign';
		}
		...
		$superseded = $order->get_meta( XPayEG_Constants::META_SUPERSEDED_SESSIONS );
	...
	private static function remember_processed( WC_Order $order, string $event_id ): void {
		...
		$processed[] = $event_id;
		$order->update_meta_data( XPayEG_Constants::META_PROCESSED_EVENTS, array_slice( $processed, -1 * self::PROCESSED_EVENTS_KEPT ) );
		$order->save();
	}
