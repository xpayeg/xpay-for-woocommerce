...
final class XPayEG_Webhook_State {
	...
	public static function reason_sentence( string $code ): string {
		switch ( $code ) {
			...
			case XPayEG_Error_Codes::WEBHOOK_SIGNATURE_MISSING:
	...
	private static function option( bool $live_mode, string $suffix ): string {
		return 'xpayeg_wh_' . ( $live_mode ? 'live' : 'test' ) . '_' . $suffix;
	}
}
