...
class XPayEG_Webhook_Configurator {
	 ...
	 * @throws XPayEG_Api_Exception When the endpoint cannot be created.
	 */
	public static function configure( string $api_key ): array {
		$client = new XPayEG_Api_Client( $api_key );
		...
		if ( ! isset( $endpoint['id'], $endpoint['secret'] ) || ! is_string( $endpoint['id'] ) || ! is_string( $endpoint['secret'] ) ) {
			...
			throw XPayEG_Api_Exception::from_api_response( array( 'message' => 'Webhook endpoint response missing id or secret' ), 502 );
		}
	 ...
	 *
	 * @param XPayEG_Api_Client $client     Client for the plane.
	 * @param string          $exclude_id The endpoint that must survive.
	 */
	private static function delete_other_endpoints_for_this_store( XPayEG_Api_Client $client, string $exclude_id ): void {
		try {
			$listing = $client->list_webhook_endpoints();
		} catch ( XPayEG_Api_Exception $e ) {
			XPayEG_Logger::event( 'webhook.dedupe_list_failed', array( 'code' => $e->get_error_code() ) );
			return;
		}
		...
		foreach ( $endpoints as $endpoint ) {
			...
			try {
				...
				XPayEG_Logger::event( 'webhook.duplicate_deleted', array( 'endpoint_id' => $endpoint['id'] ) );
			} catch ( XPayEG_Api_Exception $e ) {
				XPayEG_Logger::event(
	...
	public static function maybe_decommission( $webhook_data, string $new_api_key ): bool {
		...
		try {
			$client = new XPayEG_Api_Client( (string) $webhook_data['secret'] );
		...
		XPayEG_Logger::event( 'webhook.decommissioned', array( 'endpoint_id' => (string) $webhook_data['id'] ) );
		return true;
	}
	...
	public static function maybe_reconfigure_on_update(): void {
		...
		foreach ( array( false, true ) as $live ) {
			...
			try {
				...
				$desired = XPayEG_Event_Names::SUBSCRIBED;
				...
				XPayEG_Logger::event( 'webhook.reconfigured_on_update', array( 'live_mode' => $live ) );
			} catch ( XPayEG_Api_Exception $e ) {
				XPayEG_Logger::event(
	...
	public static function webhook_url(): string {
		return home_url( '/?wc-api=' . XPayEG_Constants::WEBHOOK_ENDPOINT );
	}
	...
	private static function settings(): array {
		$settings = get_option( 'woocommerce_' . XPayEG_Constants::GATEWAY_ID . '_settings', array() );
	...
	public static function merge_settings( array $values ): void {
		update_option( 'woocommerce_' . XPayEG_Constants::GATEWAY_ID . '_settings', array_merge( self::settings(), $values ) );
	}
}
