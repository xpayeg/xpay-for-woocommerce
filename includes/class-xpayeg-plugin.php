...
final class XPayEG_Plugin {

	/** @var XPayEG_Plugin|null */
	private static $instance = null;

	/** @var XPayEG_Gateway|null Lazily created; WooCommerce also instantiates its own copy for checkout. */
	private $gateway = null;

	public static function instance(): XPayEG_Plugin {
	...
	private function __construct() {
		$dir = XPAYEG_PLUGIN_DIR . 'includes/';

		// Constants (registries — one source of truth each).
		require_once $dir . 'constants/class-xpayeg-constants.php';
		require_once $dir . 'constants/class-xpayeg-error-codes.php';
		require_once $dir . 'constants/class-xpayeg-session-status.php';
		require_once $dir . 'constants/class-xpayeg-refund-status.php';
		require_once $dir . 'constants/class-xpayeg-charge-status.php';
		require_once $dir . 'constants/class-xpayeg-payment-methods.php';
		require_once $dir . 'constants/class-xpayeg-event-names.php';

		// Logging (loaded before anything that logs).
		require_once $dir . 'logger/class-xpayeg-redactor.php';
		require_once $dir . 'logger/class-xpayeg-logger.php';

		// Admin surfaces (order panel, settings screen).
		require_once $dir . 'admin/class-xpayeg-order-panel.php';
		require_once $dir . 'admin/class-xpayeg-admin-screen.php';

		// API layer.
		require_once $dir . 'api/class-xpayeg-api-exception.php';
		require_once $dir . 'api/class-xpayeg-signature.php';
		require_once $dir . 'api/class-xpayeg-money.php';
		require_once $dir . 'api/class-xpayeg-fx.php';
		require_once $dir . 'api/class-xpayeg-api-client.php';

		// Domain services.
		require_once $dir . 'gateway/class-xpayeg-order-lock.php';
		require_once $dir . 'gateway/class-xpayeg-checkout-service.php';
		...
		require_once $dir . 'webhooks/class-xpayeg-webhook-configurator.php';
		require_once $dir . 'webhooks/class-xpayeg-webhook-controller.php';
		require_once $dir . 'connect/class-xpayeg-connect-client.php';
		require_once $dir . 'connect/class-xpayeg-connect.php';

		// Compatibility integrations.
		require_once $dir . 'compat/class-xpayeg-wpfunnels-compat.php';

		// WooCommerce surfaces.
		require_once $dir . 'gateway/class-xpayeg-thankyou-notice.php';
		require_once $dir . 'gateway/class-xpayeg-gateway.php';
		require_once $dir . 'gateway/class-xpayeg-method-gateway.php';
		...
		if ( class_exists( \Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType::class ) ) {
			...
			require_once $dir . 'blocks/class-xpayeg-blocks-support.php';
		}
	}

	public function init(): void {
		add_filter( 'woocommerce_payment_gateways', array( $this, 'register_gateway' ) );
		if ( class_exists( 'XPayEG_Blocks_Support' ) ) {
			...
			if ( did_action( 'woocommerce_blocks_loaded' ) ) {
				...
				add_action( 'woocommerce_blocks_loaded', array( 'XPayEG_Blocks_Support', 'register' ) );
			}
		}

		// Public webhook receiver: https://<site>/?wc-api=xpayeg_webhook
		// Trust boundary: unauthenticated internet traffic — the HMAC
		// signature check inside the controller is the only gate.
		add_action( 'woocommerce_api_xpayeg_webhook', array( 'XPayEG_Webhook_Controller', 'handle' ) );
		...
		add_action( 'woocommerce_before_thankyou', array( 'XPayEG_Order_Sync', 'verify_on_thankyou' ) );
		...
		add_filter( 'woocommerce_cancel_unpaid_order', array( 'XPayEG_Order_Sync', 'should_cancel_unpaid' ), 10, 2 );

		// Activation alone cannot do it: our row is not in WooCommerce's
		// gateway map at that moment. See XPayEG_Gateway_Order.
		XPayEG_Gateway_Order::register();
		...
		$xpayeg_search_keys = static function ( $keys ) {
			$keys   = is_array( $keys ) ? $keys : array();
			$keys[] = XPayEG_Constants::META_PAYMENT_INTENT;
			$keys[] = XPayEG_Constants::META_SESSION_ID;
			return array_values( array_unique( $keys ) );
		};
		add_filter( 'woocommerce_shop_order_search_fields', $xpayeg_search_keys );
		add_filter( 'woocommerce_order_table_search_query_meta_keys', $xpayeg_search_keys );
		// Priority 20: the payment-state notice must read post-verification
		// truth. WooCommerce's own page renders untouched around it.
		add_action( 'woocommerce_before_thankyou', array( 'XPayEG_Thankyou_Notice', 'render' ), 20 );

		// Compatibility shims: WPFunnels' order-received rewrite, and
		// optimizer opt-outs on the payment-critical script tags.
		XPayEG_WPFunnels_Compat::register();
		XPayEG_Checkout_Elements::register();
		...
		add_action(
			'admin_init',
			static function () {
				if ( XPAYEG_VERSION === get_option( 'xpayeg_version_seen' ) ) {
					return;
				}
				update_option( 'xpayeg_version_seen', XPAYEG_VERSION, false );
		...
		if ( is_admin() ) {
			...
			XPayEG_Admin_Screen::register();
			add_action( 'add_meta_boxes', array( 'XPayEG_Order_Panel', 'register' ) );
			add_action( 'woocommerce_admin_order_totals_after_total', array( 'XPayEG_Order_Panel', 'render_refund_currency_note' ) );
			XPayEG_WPFunnels_Compat::register_admin();
		}
	 ...
	 * account map gets an XPayEG_Method_Gateway row, so WooCommerce's own
	...
	public function register_gateway( array $gateways ): array {
		...
		foreach ( $main->method_row_types() as $type ) {
			$gateways[] = new XPayEG_Method_Gateway( $type );
		}
		return $gateways;
	}
	...
	public static function sync_gateway_order(): void {
		...
		foreach ( $ordered as $type ) {
			$row_ids[] = XPayEG_Payment_Methods::CARD === $type ? XPayEG_Constants::GATEWAY_ID : XPayEG_Constants::GATEWAY_ID . '_' . $type;
		}
		...
		if ( ! in_array( XPayEG_Constants::GATEWAY_ID, $row_ids, true ) ) {
			array_unshift( $row_ids, XPayEG_Constants::GATEWAY_ID );
		}
		...
		foreach ( array_keys( $current ) as $id ) {
			$id = (string) $id;
			if ( XPayEG_Constants::is_xpayeg_gateway( $id ) ) {
				if ( ! $inserted ) {
	 ...
	 * XPayEG_Method_Gateway's constructor). Stripe's hide carries the same
	...
	public static function hide_method_rows_in_admin(): void {
		...
		foreach ( WC()->payment_gateways->payment_gateways as $index => $gateway ) {
			if ( $gateway instanceof XPayEG_Method_Gateway ) {
	 ...
	 */
	public function gateway(): XPayEG_Gateway {
		if ( null === $this->gateway ) {
			$this->gateway = new XPayEG_Gateway();
		} else {
