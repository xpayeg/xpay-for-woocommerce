...
final class XPayEG_Blocks_Support extends AbstractPaymentMethodType {

	/** @var array{title:string, description:string, icon:string, active:bool} */
	private $row;

	/**
	 * @param string $name Gateway id this row registers as (xpay, xpayeg_valu, …).
	...
	public static function register(): void {
		...
		add_action(
			...
			function ( $registry ) {
				...
				$registry->register(
					new self(
						XPayEG_Constants::GATEWAY_ID,
						$rows_active
							? array(
								'title'       => XPayEG_Payment_Methods::label( XPayEG_Payment_Methods::CARD ),
								'description' => XPayEG_Payment_Methods::description( XPayEG_Payment_Methods::CARD ),
								'icon'        => XPayEG_Payment_Methods::icon_url( XPayEG_Payment_Methods::CARD ),
								'method'      => XPayEG_Payment_Methods::CARD,
								// The classic card row's is_available answers
								// through the same helper, so the two
								// checkouts can never disagree about a row.
								'active'      => $gateway->method_active_for_currency( XPayEG_Payment_Methods::CARD ),
							)
							: array(
				...
				foreach ( $gateway->method_row_types() as $type ) {
					$registry->register(
						new self(
							XPayEG_Constants::GATEWAY_ID . '_' . $type,
							array(
								'title'       => XPayEG_Payment_Methods::label( $type ),
								'description' => XPayEG_Payment_Methods::description( $type ),
	...
	public function initialize(): void {
		$this->settings = get_option( 'woocommerce_' . XPayEG_Constants::GATEWAY_ID . '_settings', array() );
	}
	...
	public function get_payment_method_script_handles(): array {
		...
		wp_register_script(
			'xpayeg-blocks',
			XPAYEG_PLUGIN_URL . 'assets/js/blocks-integration.js',
			/*
			 * xpayeg-elements is the library this bundle mounts with, and the
			 * handle xpayegElementsParams is localised onto. Both were used
			 ...
			 */
			array( 'wc-blocks-registry', 'wc-settings', 'wp-element', 'wp-html-entities', 'wp-data', XPayEG_Checkout_Elements::HANDLE ),
			XPayEG_Constants::asset_version( 'assets/js/blocks-integration.js' ),
			true
		);
		return array( 'xpayeg-blocks' );
	}
