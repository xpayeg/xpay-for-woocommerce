...
class XPayEG_Refund_Service {
	...
	public function __construct( XPayEG_Api_Client $client ) {
		$this->client = $client;
	}
	...
	public function refund_order( WC_Order $order, float $amount, string $reason ): array {
		$intent_id = (string) $order->get_meta( XPayEG_Constants::META_PAYMENT_INTENT );
		if ( '' === $intent_id ) {
			throw XPayEG_Api_Exception::not_configured( 'payment intent for this order' );
		}

		$currency       = strtoupper( $order->get_currency() );
		$requested      = XPayEG_Money::to_minor( (string) $amount, $currency );
		$settles_direct = XPayEG_Constants::SETTLEMENT_CURRENCY === $currency;
		...
		$body = array(
			...
			'reason'          => XPayEG_Refund_Reason::REQUESTED_BY_CUSTOMER,
		);
		...
		if ( $settles_direct ) {
			...
			if ( null === $balance || null === $balance['rate'] ) {
				...
				throw XPayEG_Api_Exception::refund_currency_unsupported();
			}
			...
			if ( null !== $balance['refundable'] && $requested === $balance['refundable'] ) {
				...
				$settlement = XPayEG_Fx::to_settlement( $requested, $balance['rate'], $currency, XPayEG_Constants::SETTLEMENT_CURRENCY );
		...
		if ( XPayEG_Refund_Status::SUCCEEDED !== $status ) {
			XPayEG_Logger::event(
			...
			if ( in_array( $status, XPayEG_Refund_Status::IN_FLIGHT, true ) ) {
				throw XPayEG_Api_Exception::refund_pending();
			}
			throw XPayEG_Api_Exception::refund_rejected();
		}
		 ...
		 * per XPayEG_Money::session_charge(), and it answers "XPay refunded
			...
			if ( null !== $mirror && abs( $mirror - $requested ) > self::ROUNDING_TOLERANCE_MINOR ) {
				...
				$order->add_order_note(
					sprintf(
						...
						wc_price( XPayEG_Money::from_minor( $requested, $currency ), array( 'currency' => $currency ) ),
						wc_price( XPayEG_Money::from_minor( $mirror, $currency ), array( 'currency' => $currency ) ),
						wc_price( XPayEG_Money::from_minor( (int) $body['amount'], XPayEG_Constants::SETTLEMENT_CURRENCY ), array( 'currency' => XPayEG_Constants::SETTLEMENT_CURRENCY ) )
					)
				);
			}
		...
		if ( ( null !== $returned_amount && $expected !== $returned_amount ) || ( '' !== $returned_currency && $currency !== $returned_currency ) ) {
			...
			throw XPayEG_Api_Exception::refund_result_mismatch();
		}
	...
	private function presentment_balance( WC_Order $order ): ?array {
		...
		$answer = XPayEG_Refundable::from_session( $session );
	...
	private static function outstanding_minor( WC_Order $order, string $currency ): int {
		return XPayEG_Money::to_minor( (string) $order->get_total(), $currency )
			- XPayEG_Money::to_minor( (string) $order->get_total_refunded(), $currency );
	}
	...
	private function recorded_refunds( WC_Order $order ): array {
		$recorded = $order->get_meta( XPayEG_Constants::META_REFUND_IDS );
	...
	private function record_refund( WC_Order $order, string $refund_id ): void {
		...
		$recorded[] = $refund_id;
		$order->update_meta_data( XPayEG_Constants::META_REFUND_IDS, $recorded );
		$order->save();
	}
	...
	public static function mirror_charge_refunds( WC_Order $order, array $charge ): void {
		...
		$ledger = $order->get_meta( XPayEG_Constants::META_REFUND_IDS );
		$ledger = is_array( $ledger ) ? $ledger : array();

		$mirrored = 0;
		foreach ( array_reverse( $refunds ) as $refund ) { // Oldest first, so records land in issue order.
			if ( ! is_array( $refund ) || ! isset( $refund['status'] ) || XPayEG_Refund_Status::SUCCEEDED !== $refund['status'] ) {
		...
		if ( $mirrored > 0 ) {
			$order->update_meta_data( XPayEG_Constants::META_REFUND_IDS, $ledger );
			$order->save();
			XPayEG_Logger::event(
				'refund.mirrored',
				array(
	 ...
	 * mirror, settlement at the top level, per XPayEG_Money::session_charge();
	...
	private static function amount_in_order_currency( array $refund, WC_Order $order ): ?string {
		...
		if ( isset( $refund['currency'], $refund['amount'] ) && is_numeric( $refund['amount'] ) && strtoupper( (string) $refund['currency'] ) === $order_currency ) {
			return XPayEG_Money::from_minor( (int) $refund['amount'], $order_currency );
		}
		if ( isset( $refund['presentmentDetails']['currency'], $refund['presentmentDetails']['amount'] )
			...
			return XPayEG_Money::from_minor( (int) $refund['presentmentDetails']['amount'], $order_currency );
		}
		return null;
	}
	 ...
	 *
	 * @param XPayEG_Api_Exception $e The failure.
	 */
	public static function admin_message( XPayEG_Api_Exception $e ): string {
		switch ( $e->get_error_code() ) {
			case XPayEG_Error_Codes::REFUND_REJECTED:
			...
			case XPayEG_Error_Codes::REFUND_PENDING:
			...
			case XPayEG_Error_Codes::REFUND_CURRENCY_UNSUPPORTED:
			...
			case XPayEG_Error_Codes::REFUND_RESULT_MISMATCH:
			...
			case XPayEG_Error_Codes::TRANSPORT_ERROR:
			...
			case XPayEG_Error_Codes::API_RESOURCE_INVALID_STATE:
