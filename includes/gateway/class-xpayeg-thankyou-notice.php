 ...
 *
 * Truth source: XPayEG_Order_Sync re-verifies the session on the same hook
...
final class XPayEG_Thankyou_Notice {

	/**
	 * Render the payment-state notice. Hooked on woocommerce_before_thankyou
	 * at priority 20 — AFTER XPayEG_Order_Sync::verify_on_thankyou (10), so
	...
	public static function render( $order_id ): void {
		 ...
		 */
		if ( $order->has_status( 'on-hold' ) && '' !== (string) $order->get_meta( XPayEG_Constants::META_AWAITING_PAYMENT ) ) {
	...
	private static function notice( string $message ): void {
		echo '<div class="woocommerce-info xpayeg-payment-notice">' . wp_kses(
			$message,
