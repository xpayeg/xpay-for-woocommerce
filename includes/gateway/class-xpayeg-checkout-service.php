...
class XPayEG_Checkout_Service {
	...
	/**
	 * @param XPayEG_Api_Client $client                 API client for the plane.
	 ...
	 */
	public function __construct( XPayEG_Api_Client $client, ?array $accepted_types = null, ?callable $refresh_accepted_types = null ) {
		$this->client                 = $client;
	 ...
	 * @throws XPayEG_Api_Exception
	 */
	public function get_or_create_session( WC_Order $order ): array {
		$existing_id = (string) $order->get_meta( XPayEG_Constants::META_SESSION_ID );
		...
		if ( '' !== $existing_id ) {
			try {
				...
				if ( XPayEG_Error_Codes::API_RESOURCE_MISSING !== $e->get_error_code() && 404 !== $e->get_http_status() ) {
					throw $e;
				}
			}
		}
		 ...
		 * (class-xpayeg-webhook-controller.php:727-738), and an id in neither
		 ...
		 * (class-xpayeg-webhook-controller.php:100-113), which means an event
		...
		if ( '' !== $existing_id ) {
			...
			$order->update_meta_data( XPayEG_Constants::META_SUPERSEDED_SESSIONS, array_slice( array_unique( $superseded ), -10 ) );
			$order->save();
		}
	...
	private function is_paid_complete( array $session ): bool {
		return isset( $session['status'] ) && XPayEG_Session_Status::COMPLETE === $session['status']
			&& isset( $session['paymentStatus'] ) && XPayEG_Payment_Status::PAID === $session['paymentStatus'];
	}
	...
	private function apply_paid_session( WC_Order $order, array $session ): void {
		...
		$order_id = $order->get_id();
		if ( ! XPayEG_Order_Lock::acquire( $order_id, 0 ) ) {
			return;
		}
		try {
			$fresh = XPayEG_Order_Sync::reload( $order_id );
			if ( null !== $fresh && ! $fresh->is_paid() ) {
				XPayEG_Order_Sync::mark_paid( $fresh, $session, 'session-check' );
			}
		} finally {
	...
	private function reprice_session( array $session, WC_Order $order ): ?array {
		if ( XPayEG_Session_Status::OPEN !== ( $session['status'] ?? '' ) || ! empty( $session['isExpired'] ) || self::is_emptied( $session ) ) {
			return null;
		}
		...
		$charge = XPayEG_Money::session_charge( $session );
		if ( null === $charge ) {
			return null;
		}
		...
		$total = XPayEG_Money::to_minor( $order->get_total(), $currency );
		$seq   = (int) $order->get_meta( XPayEG_Constants::META_REPRICE_SEQ ) + 1;
		...
		$order->update_meta_data( XPayEG_Constants::META_REPRICE_SEQ, (string) $seq );
	...
	private function is_reusable( array $session, WC_Order $order ): bool {
		...
		// settlement second, per XPayEG_Money::session_charge().
		$charge = XPayEG_Money::session_charge( $session );
		if ( null === $charge ) {
			return false;
		}
		...
		return XPayEG_Session_Status::OPEN === $status
			&& ! $expired
			&& strtoupper( $order->get_currency() ) === $charge['currency']
			&& XPayEG_Money::to_minor( $order->get_total(), $order->get_currency() ) === $charge['amount']
	...
	private function create_session( WC_Order $order ): array {
		$attempt  = (int) $order->get_meta( XPayEG_Constants::META_ATTEMPT ) + 1;
		$currency = strtoupper( $order->get_currency() );
		if ( array() === $this->accepted_types ) {
			throw XPayEG_Api_Exception::payment_methods_unavailable();
		}

		// xpayeg_session_id is deliberately never read by plugin code: it puts
		...
		$return_url = add_query_arg(
			array( 'xpayeg_session_id' => '{CHECKOUT_SESSION_ID}' ),
		...
		$body = array(
			...
			'uiMode'          => 'custom',
			'currency'        => $currency,
			'lineItems'       => self::line_items( $order, XPayEG_Money::to_minor( $order->get_total(), $currency ), $currency ),
		...
		try {
			 ...
			 */
			if ( isset( $body['customerId'] ) && XPayEG_Error_Codes::API_RESOURCE_MISSING === $e->get_error_code() ) {
				// A stored customer link the platform no longer knows:
				// clear it so the next checkout re-creates, retry without.
				delete_user_meta( $order->get_user_id(), XPayEG_Constants::customer_user_meta_key( $this->client->is_live_mode() ) );
				XPayEG_Logger::event(
		...
		if ( empty( $session['id'] ) || empty( $session['clientSecret'] ) ) {
			throw XPayEG_Api_Exception::from_api_response( array( 'message' => 'Session response missing id or clientSecret' ), 502 );
		}

		$order->update_meta_data( XPayEG_Constants::META_SESSION_ID, (string) $session['id'] );
		$order->update_meta_data( XPayEG_Constants::META_CLIENT_SECRET, (string) $session['clientSecret'] );
		$order->update_meta_data( XPayEG_Constants::META_ATTEMPT, $attempt );
		$order->save();
	 ...
	 *      session and XPayEG_Order_Sync stores it for next time.
	...
	private function apply_customer_fields( array $body, WC_Order $order ): array {
		...
		if ( $user_id > 0 && ! isset( $body['customerId'] ) ) {
			$stored = (string) get_user_meta( $user_id, XPayEG_Constants::customer_user_meta_key( $this->client->is_live_mode() ), true );
