 ...
 * state.
 *
 * It DOES nest, in one place. process_payment holds this lock
 * (class-xpayeg-gateway.php:689) across a section that reaches
 * apply_paid_session(), which acquires and releases the SAME named lock
 * for the same order (class-xpayeg-checkout-service.php:367, :376).
 *
 * Pre-5.7.5 MySQL's limit is one lock per connection across DIFFERENT
 * names, and both of these are the same name, so that limit is still not
 * reached. It is reached by accident rather than by the file's old claim
 * that nothing nests, which was simply untrue.
 *
 * On 5.7.5+ GET_LOCK is recursive and RELEASE_LOCK decrements, so the
 * inner release leaves the outer section still holding. On older MySQL it
 * would end the outer section early. Nobody has established what that
 * costs, because the inner section's own re-read and is_paid() check are
 * what actually bound the race it guards — verify before relying on the
 * lock alone there.
 *
