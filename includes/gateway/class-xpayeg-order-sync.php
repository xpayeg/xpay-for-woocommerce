...
class XPayEG_Order_Sync {
	...
	const META_CANCEL_DEFERRED = '_xpayeg_cancel_deferred';
	...
	public static function should_cancel_unpaid( $should_cancel, $order ): bool {
		...
		if ( ! XPayEG_Constants::is_xpayeg_gateway( (string) $order->get_payment_method() ) ) {
			return true;
		}
		if ( $order->is_paid() ) {
			return false;
		...
		if ( '' === (string) $order->get_meta( XPayEG_Constants::META_SESSION_ID ) ) {
			return true;
		}
		 ...
		 * @param WC_Order $order   The unpaid order.
		 */
		$grace = (int) apply_filters( 'xpayeg_unpaid_order_grace_seconds', self::UNPAID_GRACE_SECONDS, $order );
		...
		XPayEG_Logger::event( 'order.cancel_deferred', array( 'order_id' => $order->get_id() ) );
	...
	public static function apply_completed( WC_Order $order, array $session, string $via ): void {
		...
		if ( XPayEG_Payment_Status::UNPAID === $status ) {
	...
	public static function mark_paid( WC_Order $order, array $session, string $via ): void {
		...
		if ( '' !== $payment_status && XPayEG_Payment_Status::PAID !== $payment_status && XPayEG_Payment_Status::NO_PAYMENT_REQUIRED !== $payment_status ) {
			XPayEG_Logger::error(
		...
		if ( '' !== $intent_id ) {
			$order->update_meta_data( XPayEG_Constants::META_PAYMENT_INTENT, $intent_id );
		}
		...
		if ( isset( $session['livemode'] ) ) {
			add_option( XPayEG_Constants::first_paid_option( (bool) $session['livemode'] ), time(), '', false );
		}

		$parked = '' !== (string) $order->get_meta( XPayEG_Constants::META_PAID_AFTER_CANCEL )
			|| '' !== (string) $order->get_meta( XPayEG_Constants::META_SUPERSEDED_PARKED );
		if ( $order->has_status( 'cancelled' ) || $parked ) {
			...
			$order->update_meta_data( XPayEG_Constants::META_PAID_AFTER_CANCEL, (string) time() );
			XPayEG_Logger::error(
		...
		$order->payment_complete( '' !== $intent_id ? $intent_id : (string) $order->get_meta( XPayEG_Constants::META_SESSION_ID ) );
	 ...
	 * XPayEG_Money::session_charge(). Missing/malformed fields answer false:
	...
	private static function charged_amount_disagrees( array $session, WC_Order $order ): bool {
		...
		$expected = XPayEG_Money::to_minor( $order->get_total(), $order->get_currency() );
	...
	private static function mismatch_note( array $session, WC_Order $order ): string {
		...
		return sprintf(
			...
			wc_price( XPayEG_Money::from_minor( (string) $amount, $currency ), array( 'currency' => $currency ) ),
	...
	private static function cancelled_note( array $session ): string {
		...
		$amount   = null === $charge
			? ''
			: wc_price( XPayEG_Money::from_minor( (string) $charge['amount'], $charge['currency'] ), array( 'currency' => $charge['currency'] ) );
		$sentence = '' === $amount
	...
	private static function remember_customer( WC_Order $order, array $session ): void {
		...
		$order->update_meta_data( XPayEG_Constants::META_CUSTOMER_ID, $customer_id );

		$user_id = $order->get_user_id();
		if ( $user_id > 0 && isset( $session['livemode'] ) ) {
			update_user_meta( $user_id, XPayEG_Constants::customer_user_meta_key( (bool) $session['livemode'] ), $customer_id );
		}
	}

	/**
	 * Re-read an order from storage, discarding this request's cached copy.
	 * Call ONLY while holding the order's XPayEG_Order_Lock: the point is to
	...
	public static function apply_superseded_paid( WC_Order $order, array $session ): void {
		$paid = isset( $session['paymentStatus'] ) && XPayEG_Payment_Status::PAID === $session['paymentStatus'];
		if ( ! $paid ) {
		 ...
		 * look at the first.
		 */
		$order->update_meta_data( XPayEG_Constants::META_SUPERSEDED_PARKED, (string) time() );
	...
	public static function mark_awaiting_payment( WC_Order $order, array $session ): void {
		...
		if ( '' !== (string) $order->get_meta( XPayEG_Constants::META_AWAITING_PAYMENT ) ) {
			return;
		}
	...
	public static function mark_expired( WC_Order $order, array $session = array() ): void {
		...
		XPayEG_Logger::event( 'order.session_expired', array( 'order_id' => $order->get_id() ) );
	}
	...
	public static function verify_on_thankyou( $order_id ): void {
		$order = wc_get_order( $order_id );
		if ( ! $order instanceof WC_Order || ! XPayEG_Constants::is_xpayeg_gateway( (string) $order->get_payment_method() ) || $order->is_paid() ) {
			return;
		}

		$session_id = (string) $order->get_meta( XPayEG_Constants::META_SESSION_ID );
		if ( '' === $session_id ) {
			return;
		}

		try {
			$client  = XPayEG_Plugin::instance()->gateway()->api_client();
			$session = $client->get_checkout_session( $session_id, XPayEG_Api_Client::SHOPPER_READ_TIMEOUT_SECONDS );
		...
		$paid = isset( $session['paymentStatus'] ) && XPayEG_Payment_Status::PAID === $session['paymentStatus']
			&& isset( $session['status'] ) && XPayEG_Session_Status::COMPLETE === $session['status'];
