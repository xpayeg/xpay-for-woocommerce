 ...
 *      xpayeg_confirm, telling the browser to confirm before it follows
 *      the redirect.
 ...
 * (XPayEG_Order_Sync) own all paid/expired transitions.
 *
...
class XPayEG_Gateway extends WC_Payment_Gateway {

	/** @var XPayEG_Api_Client|null Lazy — settings may be incomplete on admin screens. */
	private $client = null;
	...
	public function __construct() {
		$this->id = XPayEG_Constants::GATEWAY_ID;
		/*
		...
		$this->icon = XPAYEG_PLUGIN_URL . 'assets/images/xpay-icon.svg';
	...
	public function init_form_fields(): void {
		$webhook_url = home_url( '/?wc-api=' . XPayEG_Constants::WEBHOOK_ENDPOINT );
		...
		$this->form_fields = array(
			...
			// Key must match XPayEG_WPFunnels_Compat::SETTING_KEY.
			...
			'debug'                             => array(
				...
				'type'    => 'checkbox',
				'label'   => __( 'Write redacted diagnostic logs (WooCommerce → Status → Logs, source "xpayeg")', 'xpay-for-woocommerce' ),
				'default' => 'no',
			),
		);
	}
	...
	protected function base_available(): bool {
		 ...
		 * hardcoded platform enum (XPayEG_Money::DECIMALS) remains only as
		...
		$supported = array() !== $map ? array_keys( $map ) : array_keys( XPayEG_Money::DECIMALS );
	...
	public function account_methods_map(): array {
		$map = get_option( XPayEG_Constants::account_methods_option( ! $this->is_test_mode() ), array() );
		if ( ! is_array( $map ) ) {
			return array();
		}
		$clean = array();
	...
	public function ordered_method_types(): array {
		...
		$saved     = get_option( XPayEG_Constants::OPTION_METHOD_ORDER, array() );
	...
	public function method_row_types(): array {
		...
		foreach ( $this->enabled_method_types() as $type ) {
			if ( XPayEG_Payment_Methods::CARD !== $type ) {
				$types[] = $type;
			}
		}
		return $types;
	 ...
	 * (XPayEG_Method_Gateway). Without the map it stays the single "XPay"
	 * row and the fields render the account's full method list themselves.
	 */
	public function payment_fields(): void {
		XPayEG_Checkout_Elements::render_mount( $this->method_rows_active() ? XPayEG_Payment_Methods::CARD : null );
	}
	...
	public function is_available() {
		...
		return null === $accepted || in_array( XPayEG_Payment_Methods::CARD, $accepted, true );
	}
	...
	public function get_title() {
		if ( ! is_admin() && $this->method_rows_active() ) {
			...
			return apply_filters( 'woocommerce_gateway_title', XPayEG_Payment_Methods::label( XPayEG_Payment_Methods::CARD ), $this->id );
		}
	...
	private static function already_proved( string $secret, string $publishable, bool $test_mode ): bool {
		...
		return hash_equals(
			...
			XPayEG_Constants::key_fingerprint( $secret, $publishable )
		);
	}

	/**
	 * @throws XPayEG_Api_Exception When no key is configured.
	 */
	public function api_client(): XPayEG_Api_Client {
		if ( null === $this->client ) {
			$this->client = new XPayEG_Api_Client( $this->api_key() );
		}
		return $this->client;
	}
	...
	public function get_icon() {
		if ( ! is_admin() && $this->method_rows_active() ) {
			$icon = '<img src="' . esc_url( XPayEG_Payment_Methods::icon_url( XPayEG_Payment_Methods::CARD ) ) . '" class="xpayeg-method-icon xpayeg-method-icon--card" alt="' . esc_attr( XPayEG_Payment_Methods::label( XPayEG_Payment_Methods::CARD ) ) . '" />';
			// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WooCommerce core filter.
			return apply_filters( 'woocommerce_gateway_icon', $icon, $this->id );
		}
		if ( XPayEG_Constants::GATEWAY_ID === $this->id ) {
			return '';
		}
	...
	public function get_connection_url( string $return_url = '' ): string {
		return add_query_arg( 'xpayeg-setup', '1', admin_url( 'admin.php?page=wc-settings&tab=checkout&section=' . $this->id ) );
	}
	...
	public function process_admin_options() {
		 ...
		 */
		$after_decommission = XPayEG_Webhook_Configurator::decommission_after_key_update( get_option( $this->get_option_key(), array() ) );
	...
	public function validate_and_provision(): array {
		...
		$key_is_live  = XPayEG_Api_Client::is_live_key( $key );
		 ...
		 * point: is_live_key( '' ) is false (`class-xpayeg-api-client.php:54`),
		...
		if ( XPayEG_Api_Client::is_live_key( $publishable ) !== $mode_is_live ) {
			delete_option( XPayEG_Constants::OPTION_KEY_VALIDATED );
			$notices[] = array(
				'type' => 'error',
		...
		try {
			...
			} elseif ( 403 === $status && XPayEG_Error_Codes::API_PERMISSION_DENIED === $e->get_error_code() ) {
		...
		update_option(
			...
			array(
				...
				'fingerprint'  => XPayEG_Constants::key_fingerprint( $key, $publishable ),
			),
			false
		);
	...
	private function save_enabled_methods(): void {
		...
		if ( ! isset( $_POST['xpayeg_methods_present'] ) ) {
			return;
		}

		$posted = array();
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- see above.
		if ( isset( $_POST['xpayeg_method_enabled'] ) && is_array( $_POST['xpayeg_method_enabled'] ) ) {
			...
			$posted = array_map( 'sanitize_key', array_filter( wp_unslash( $_POST['xpayeg_method_enabled'] ), 'is_string' ) );
		}
	...
	private function cache_account_facts( array $account ): void {
		...
		// re-read).
		update_option( XPayEG_Constants::account_checked_option( $live ), time(), false );
		...
		if ( array() !== $map ) {
			update_option( XPayEG_Constants::account_methods_option( $live ), $map, false );
	...
	public function maybe_refresh_account_facts(): void {
		...
		$option  = XPayEG_Constants::account_checked_option( ! $this->is_test_mode() );
		...
		try {
			$account = $this->api_client()->get_account( XPayEG_Api_Client::SHOPPER_READ_TIMEOUT_SECONDS );
		} catch ( XPayEG_Api_Exception $e ) {
			XPayEG_Logger::event( 'account.refresh_failed', array( 'code' => $e->get_error_code() ) );
			return;
		}

		$this->cache_account_facts( $account );
		XPayEG_Logger::event( 'account.refreshed', array( 'live_mode' => ! $this->is_test_mode() ) );
	}
	...
	private function maybe_configure_webhook( array $account ): ?string {
		...
		$recorded = XPayEG_Webhook_Configurator::endpoint_data( $live );
		...
		if ( ! in_array( XPayEG_Webhook_Configurator::REQUIRED_PERMISSION, $permissions, true ) ) {
		...
		try {
			...
			XPayEG_Logger::error( 'webhook.configure_failed', array( 'code' => $e->get_error_code() ) );
	 ...
	 * the settings form (XPayEG_Admin_Screen). Every control posts the
	 * standard woocommerce_xpayeg_* field names, so process_admin_options
	...
	public function admin_options() {
		 ...
		 */
		XPayEG_Plugin::sync_gateway_order();
		XPayEG_Admin_Screen::render( $this );
	}
	...
	public function get_transaction_url( $order ) {
		$intent = (string) $order->get_meta( XPayEG_Constants::META_PAYMENT_INTENT );
		if ( '' === $intent ) {
			$intent = (string) $order->get_transaction_id();
		}

		$url = XPayEG_Constants::payment_dashboard_url( $intent, ! $this->is_test_mode() );
		if ( '' === $url ) {
	...
	public function process_payment( $order_id ) {
		 ...
		 * XPayEG_Checkout_Service::get_or_create_session.
		 *
		...
		try {
			 ...
			 * XPayEG_Api_Client's constructor throws on an empty key
			 * (class-xpayeg-api-client.php:42). is_available() should keep an
		 ...
		 * it. Non-fatal if it cannot be had: XPayEG_Order_Lock answers true
		 * when the host has no GET_LOCK at all, and the platform's own
		 * idempotency keys still collapse duplicate writes.
		 */
		$locked = XPayEG_Order_Lock::acquire( (int) $order->get_id(), 5 );
		...
		try {
			...
			if ( $locked ) {
				XPayEG_Order_Lock::release( (int) $order->get_id() );
			}
		}
		...
		if ( isset( $session['status'] ) && XPayEG_Session_Status::COMPLETE === $session['status'] ) {
			return array(
				'result'   => 'success',
		...
		return array(
			...
			'xpayeg_secret'    => $client_secret,
			'xpayeg_order_id'  => (string) $order->get_id(),
			/*
			 ...
			 * order.
			 */
			'xpayeg_order_key' => (string) $order->get_order_key(),
		);
	}
	 ...
	 * XPayEG_Checkout_Elements::enqueue(), which serves this endpoint too.
	 *
	 * @param int $order_id Order being paid.
	 */
	public function receipt_page( $order_id ): void {
		...
		XPayEG_Checkout_Elements::render_mount();
		echo '<button type="button" class="button alt" data-xpayeg-pay>' . esc_html__( 'Pay now', 'xpay-for-woocommerce' ) . '</button>';
	}
	 ...
	 *      against; XPayEG_Refund_Service throws not_configured on sight.
	 ...
	 *      (XPayEG_Fx) and checked against the presentment mirror before it
	...
	public function can_refund_order( $order ) {
		...
		if ( '' === (string) $order->get_meta( XPayEG_Constants::META_PAYMENT_INTENT ) ) {
			return false;
		}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( ! $order instanceof WC_Order || null === $amount ) {
			return new WP_Error( 'xpayeg_refund', __( 'Refund amount is required.', 'xpay-for-woocommerce' ) );
		}

		try {
			$service = new XPayEG_Refund_Service( $this->api_client() );
			$service->refund_order( $order, (float) $amount, (string) $reason );
			return true;
		} catch ( XPayEG_Api_Exception $e ) {
			return new WP_Error( 'xpayeg_refund_' . $e->get_error_code(), XPayEG_Refund_Service::admin_message( $e ) );
		}
	}
}
