...
final class XPayEG_Gateway_Order {
	 ...
	 */
	const OPTION_APPLIED = 'xpayeg_gateway_order_applied';
	...
	public static function apply_default(): void {
		...
		foreach ( $order as $id => $position ) {
			$id = (string) $id;
			if ( XPayEG_Constants::GATEWAY_ID === $id ) {
				// The combined row is the one a shopper should meet first.
				// A store carrying older xpayeg_* rows must not push it down
				// just because they happen to sort earlier today.
				$primary[] = $id;
			} elseif ( XPayEG_Constants::is_xpayeg_gateway( $id ) ) {
				$ours[] = $id;
			} else {
				$others[] = $id;
			}
