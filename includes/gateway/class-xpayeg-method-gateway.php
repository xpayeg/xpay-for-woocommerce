<?php
/**
 * XPayEG_Method_Gateway
 *
 * One WooCommerce checkout row for one XPay payment method (ValU, Fawry,
 * …). The main XPayEG_Gateway is the card row; every other method the
 ...
 * what makes this row a row: its id (`xpayeg_<type>`), its label, icon and
 ...
 * XPayEG_Plugin::hide_method_rows_in_admin): merchants manage one XPay
...
class XPayEG_Method_Gateway extends WC_Payment_Gateway {
	...
	public function __construct( string $type ) {
		...
		$this->id          = XPayEG_Constants::GATEWAY_ID . '_' . $type;
		 ...
		 */
		$this->title       = XPayEG_Payment_Methods::label( $type );
		$this->description = XPayEG_Payment_Methods::description( $type );
		$this->icon        = XPayEG_Payment_Methods::icon_url( $type );
	...
	private function main_gateway(): XPayEG_Gateway {
	...
	public function payment_fields(): void {
		XPayEG_Checkout_Elements::render_mount( $this->method_type );
	}
	 ...
	 * XPayEG_Checkout_Elements registers) — the same pattern as Stripe's
	 * classed payment icons. '' when no licensed artwork ships.
	 */
	public function get_icon() {
		$url = XPayEG_Payment_Methods::icon_url( $this->method_type );
		if ( '' === $url ) {
			return '';
		}
		$icon = '<img src="' . esc_url( $url ) . '" class="xpayeg-method-icon xpayeg-method-icon--' . esc_attr( $this->method_type ) . '" alt="' . esc_attr( XPayEG_Payment_Methods::label( $this->method_type ) ) . '" />';
