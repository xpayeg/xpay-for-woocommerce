...
final class XPayEG_Checkout_Elements {
	 ...
	 */
	const NONCE_ACTION = 'woocommerce_xpayeg_checkout_elements';

	/** Script handle for the mount module. */
	const HANDLE = 'xpayeg-elements';

	/** Script handle for the page driver that mounts and pays. */
	const DRIVER_HANDLE = 'xpayeg-checkout-driver';

	/** Style handle carrying the row-icon rule. */
	const STYLE_HANDLE = 'xpayeg-checkout';
	...
	public static function register(): void {
		...
		foreach ( array( 'outcome', 'order_session' ) as $action ) {
			add_action( 'wp_ajax_xpayeg_elements_' . $action, array( __CLASS__, 'handle_' . $action ) );
			add_action( 'wp_ajax_nopriv_xpayeg_elements_' . $action, array( __CLASS__, 'handle_' . $action ) );
		}
	}
	 ...
	 *
	 * Split out because the Blocks bundle declares `xpayeg-elements` as a
	 ...
	 * page and logged:
	 *
	 *   Payment gateway with handle 'xpayeg-blocks' has been deactivated in
	 *   Cart and Checkout blocks because its dependency 'xpayeg-elements' is
	 *   not registered.
	 *
	...
	public static function register_scripts(): void {
		 ...
		 */
		wp_register_style( self::STYLE_HANDLE, false, array(), XPAYEG_VERSION );
		wp_add_inline_style(
			self::STYLE_HANDLE,
			'ul.payment_methods li img.xpayeg-method-icon{float:right;max-height:24px;max-width:56px;margin:0;padding-left:3px}'
			/*
			 ...
			 */
			. 'ul.payment_methods li img.xpayeg-method-icon--fawry{transform:scale(1.25);transform-origin:right center}'
		);

		wp_register_script(
			self::HANDLE,
			XPAYEG_PLUGIN_URL . 'assets/js/checkout-elements.js',
			array(),
			XPayEG_Constants::asset_version( 'assets/js/checkout-elements.js' ),
			true
		);

		wp_register_script(
			self::DRIVER_HANDLE,
			XPAYEG_PLUGIN_URL . 'assets/js/checkout-driver.js',
			...
			XPayEG_Constants::asset_version( 'assets/js/checkout-driver.js' ),
			true
		);
	}
	...
	public static function enqueue(): void {
		 ...
		 *
		 * Both checkouts read xpayegElementsParams, and the Blocks bundle
		...
		wp_localize_script(
			...
			array(
				...
				'sdkUrl'         => XPayEG_Constants::sdk_url(),
	 ...
	 *
	 * @param XPayEG_Gateway $gateway Configured gateway.
	 */
	private static function enqueue_pay_page( XPayEG_Gateway $gateway ): void {
		global $wp;
		...
		wp_enqueue_script(
			'xpayeg-pay-page',
			XPAYEG_PLUGIN_URL . 'assets/js/pay-page.js',
			array( self::HANDLE ),
			XPayEG_Constants::asset_version( 'assets/js/pay-page.js' ),
			true
		);
	 ...
	 * @return string[]
	 */
	private static function row_names( XPayEG_Gateway $gateway ): array {
		$names = array( XPayEG_Constants::GATEWAY_ID );
		foreach ( $gateway->method_row_types() as $type ) {
			$names[] = XPayEG_Constants::GATEWAY_ID . '_' . $type;
		}
		return $names;
	}
	 ...
	 * @param XPayEG_Gateway $gateway Configured gateway.
	 */
	private static function color_mode( XPayEG_Gateway $gateway ): string {
	...
	public static function render_mount( ?string $method = null ): void {
		...
		$description = null !== $method
			? XPayEG_Payment_Methods::description( $method )
			: (string) $gateway->get_option( 'description', '' );
		if ( '' !== $description ) {
			echo '<p class="xpayeg-el__description">' . esc_html( $description ) . '</p>';
		}
		 ...
		 */
		$amount = self::cart_total_minor();
		echo '<div class="xpayeg-el" data-xpayeg-elements data-xpayeg-method="' . esc_attr( (string) $method ) . '" data-xpayeg-amount="' . esc_attr( (string) ( null === $amount ? 0 : $amount ) ) . '">';
		echo '<div class="xpayeg-el__mount"></div>';
		echo '<div class="xpayeg-el__notice" data-xpayeg-elements-error hidden role="alert"></div>';
	...
	public static function handle_outcome(): void {
		...
		try {
			$session = XPayEG_Plugin::instance()->gateway()->api_client()->get_checkout_session(
				$session_id,
				XPayEG_Api_Client::SHOPPER_READ_TIMEOUT_SECONDS
			);
		} catch ( XPayEG_Api_Exception $e ) {
	...
	private static function owned_session_id( int $order_id, string $order_key ): string {
		...
		return (string) $order->get_meta( XPayEG_Constants::META_SESSION_ID );
	}
	...
	private static function verdict_for( ?array $session ): string {
		...
		$payment_status = isset( $session['paymentStatus'] ) ? (string) $session['paymentStatus'] : '';
		if ( XPayEG_Payment_Status::PAID === $payment_status ) {
			return 'paid';
		}
		 ...
		 */
		$open = XPayEG_Session_Status::OPEN === ( isset( $session['status'] ) ? (string) $session['status'] : '' )
			&& empty( $session['isExpired'] );

		if ( XPayEG_Payment_Status::UNPAID !== $payment_status || ! $open ) {
			return 'unknown';
		}
	 ...
	 * XPayEG_Refundable::from_session(): a payload that did not expand its
	...
	private static function attempt_in_flight( array $session ): ?bool {
		...
		foreach ( $intent['charges'] as $charge ) {
			...
			if ( XPayEG_Charge_Status::PENDING === (string) $charge['status'] ) {
				return true;
			}
		}
	 ...
	 *
	 * Read through XPayEG_Money so the same string-based conversion that
	...
	private static function cart_total_minor(): ?int {
		...
		try {
			$minor = XPayEG_Money::to_minor( (string) $total, get_woocommerce_currency() );
	 ...
	 *
	 * Through XPayEG_Plugin::gateway(), never out of WC()'s registry: the
	 ...
	 * per question.
	 */
	private static function gateway(): ?XPayEG_Gateway {
		if ( ! class_exists( 'WC_Payment_Gateway' ) ) {
			return null;
		}
		return XPayEG_Plugin::instance()->gateway();
	}
}
