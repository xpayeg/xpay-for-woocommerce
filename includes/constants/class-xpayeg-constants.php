...
final class XPayEG_Constants {
	...
	public static function asset_version( string $rel_path ): string {
		$path  = XPAYEG_PLUGIN_DIR . $rel_path;
		$mtime = is_file( $path ) ? filemtime( $path ) : false;
		return XPAYEG_VERSION . ( false === $mtime ? '' : '.' . $mtime );
	}

	/**
	 * Business API base. Overridable for staging via the XPAYEG_API_BASE
	...
	public static function api_base(): string {
		if ( defined( 'XPAYEG_API_BASE' ) && is_string( XPAYEG_API_BASE ) ) {
			return untrailingslashit( XPAYEG_API_BASE );
		}
	 ...
	 * api_base() so the XPAYEG_API_BASE staging override carries the
	 ...
	 * anti-forgery proof.
	 */
	const OPTION_CONNECT_CLIENT = 'xpayeg_connect_client';
	const OPTION_CONNECT_FLOW   = 'xpayeg_connect_flow';
	...
	public static function sdk_url(): string {
		if ( defined( 'XPAYEG_SDK_URL' ) && is_string( XPAYEG_SDK_URL ) ) {
			return XPAYEG_SDK_URL;
		}
	 ...
	 */
	const META_SESSION_ID     = '_xpayeg_session_id';
	const META_CLIENT_SECRET  = '_xpayeg_client_secret';
	const META_PAYMENT_INTENT = '_xpayeg_payment_intent_id';
	const META_ATTEMPT        = '_xpayeg_session_attempt';
	 ...
	 */
	const META_REPRICE_SEQ      = '_xpayeg_reprice_seq';
	const META_PROCESSED_EVENTS = '_xpayeg_processed_events';
	const META_CUSTOMER_ID      = '_xpayeg_customer_id';
	 ...
	 *
	 * Written by XPayEG_Order_Sync::mark_awaiting_payment() so a redelivered
	 ...
	 */
	const META_AWAITING_PAYMENT = '_xpayeg_awaiting_payment';
	 ...
	 */
	const META_PAID_AFTER_CANCEL = '_xpayeg_paid_after_cancel';
	 ...
	 */
	const META_SUPERSEDED_PARKED = '_xpayeg_superseded_parked';
	 ...
	 */
	const META_SUPERSEDED_SESSIONS = '_xpayeg_superseded_sessions';
	 ...
	 */
	const META_REFUND_IDS = '_xpayeg_refund_ids';
	 ...
	 * proved.
	 */
	const OPTION_KEY_VALIDATED = 'xpayeg_key_validated';
	 ...
	 */
	const OPTION_FIRST_PAID_AT_TEST = 'xpayeg_first_paid_at_test';
	const OPTION_FIRST_PAID_AT_LIVE = 'xpayeg_first_paid_at_live';
	 ...
	 */
	const OPTION_MERCHANT_ID_TEST = 'xpayeg_merchant_id_test';
	const OPTION_MERCHANT_ID_LIVE = 'xpayeg_merchant_id_live';
	 ...
	 * cache outlives XPayEG_Gateway::ACCOUNT_CACHE_SECONDS. Read by the
	 ...
	 */
	const OPTION_ACCOUNT_METHODS_TEST = 'xpayeg_account_methods_test';
	const OPTION_ACCOUNT_METHODS_LIVE = 'xpayeg_account_methods_live';
	 ...
	 * refresh measures against — see XPayEG_Gateway::ACCOUNT_CACHE_SECONDS.
	 *
	 * @param bool $live_mode Which plane.
	 */
	public static function account_checked_option( bool $live_mode ): string {
		return 'xpayeg_account_checked_at_' . ( $live_mode ? 'live' : 'test' );
	}
	 ...
	 * rewrite it.
	 */
	const OPTION_ENABLED_METHODS = 'xpayeg_enabled_methods';
	const OPTION_METHOD_ORDER    = 'xpayeg_method_order';
	...
	public static function customer_user_meta_key( bool $live_mode ): string {
		return $live_mode ? '_xpayeg_customer_id_live' : '_xpayeg_customer_id_test';
	}

	/** Gateway id — also the settings option suffix (woocommerce_xpayeg_settings). */
	const GATEWAY_ID = 'xpayeg';
	 ...
	 * price in any of the
	 * currencies XPayEG_Money knows; that price is converted at a rate locked
	...
	/**
	 * True for the combined gateway AND its per-method rows (xpayeg_card,
	 * xpayeg_valu, …). Every "was this order paid through us" check must use
	 * this, not an exact GATEWAY_ID comparison — a ValU-row order carries
	 * payment_method 'xpayeg_valu' and is just as much ours.
	 *
	 * @param string $gateway_id A WooCommerce payment method id.
	 */
	public static function is_xpayeg_gateway( string $gateway_id ): bool {
		return self::GATEWAY_ID === $gateway_id || 0 === strpos( $gateway_id, self::GATEWAY_ID . '_' );
	}

	/** WC-API endpoint slug for the webhook receiver. */
	const WEBHOOK_ENDPOINT = 'xpayeg_webhook';
	 ...
	 */
	public static function is_allowed_xpayeg_url( string $url ): bool {
		...
		if ( defined( 'XPAYEG_API_BASE' ) && in_array( $host, $local_hosts, true ) ) {
