 ...
 * XPay's OAuth issuer. Kept apart from XPayEG_Api_Client on purpose —
 ...
 * callers: it is short-lived and the xpayeg_* fields are the
...
final class XPayEG_Connect_Client {
	 ...
	 * @throws XPayEG_Api_Exception When registration fails or the response
	 ...
	 * @return array Decoded token response (the xpayeg_* fields are the
	 *               deliverable; the access_token inside is discarded by
	 *               every caller).
	 * @throws XPayEG_Api_Exception When the exchange is refused or fails.
	 */
	 ...
	 * way XPayEG_Api_Client logs its calls.
	 *
	 ...
	 * @throws XPayEG_Api_Exception On transport failure or any non-2xx.
	 */
	private static function request( string $path, string $body, string $content_type, array $log_context ): array {
		$url = XPayEG_Constants::oauth_base() . $path;
		...
		$response = wp_remote_post(
			...
			array(
				'headers' => array(
					...
					'User-Agent'   => 'XPay-WooCommerce/' . XPAYEG_VERSION . '; ' . home_url(),
				),
				'body'    => $body,
		...
		if ( is_wp_error( $response ) ) {
			...
			throw XPayEG_Api_Exception::transport( 'Could not reach the XPay OAuth service' );
		}
		...
		$code    = isset( $json['error'] ) && is_string( $json['error'] ) && '' !== trim( $json['error'] )
			? trim( $json['error'] )
			: XPayEG_Error_Codes::CONNECT_EXCHANGE_FAILED;
