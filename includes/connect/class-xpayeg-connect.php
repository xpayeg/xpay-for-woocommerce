...
final class XPayEG_Connect {

	/** WC-API slug of the OAuth callback (the registered redirect URI). */
	const CALLBACK_ENDPOINT = 'xpayeg_connect';
	...
	public static function settings_url(): string {
		return admin_url( 'admin.php?page=wc-settings&tab=checkout&section=' . XPayEG_Constants::GATEWAY_ID );
	}
	 ...
	 * @throws XPayEG_Api_Exception When registration fails.
	 */
	public static function begin( bool $live ): string {
		...
		try {
			...
			throw XPayEG_Api_Exception::connect_randomness_failed();
		}
		...
		update_option(
			XPayEG_Constants::OPTION_CONNECT_FLOW,
			array(
				'state'      => $state,
				'verifier'   => $verifier,
				'live'       => $live,
		...
		return add_query_arg(
			...
			XPayEG_Constants::oauth_base() . '/oauth2/authorize'
		);
	}
	...
	private static function ensure_client(): array {
		$stored = get_option( XPayEG_Constants::OPTION_CONNECT_CLIENT, null );
		...
		$registered = XPayEG_Connect_Client::register_client( $body );
		...
		update_option( XPayEG_Constants::OPTION_CONNECT_CLIENT, $client, false );

		XPayEG_Logger::event( 'connect.client_registered', array( 'client_id' => $client['client_id'] ) );
	...
	public static function handle_callback(): string {
		...
		$flow = get_option( XPayEG_Constants::OPTION_CONNECT_FLOW, null );
		 ...
		 */
		if ( ! delete_option( XPayEG_Constants::OPTION_CONNECT_FLOW ) ) {
			XPayEG_Logger::event( 'connect.duplicate_callback', array() );
			return self::settings_url();
		}

		$flow_error = self::flow_error( $flow, $state, get_current_user_id(), time() );
		if ( null !== $flow_error ) {
			XPayEG_Logger::error( 'connect.flow_rejected', array( 'reason' => $flow_error ) );
			self::store_notices(
				array(
					array(
						'type' => 'error',
		 ...
		 */
		if ( '' !== $iss && ! hash_equals( XPayEG_Constants::oauth_base(), $iss ) ) {
			XPayEG_Logger::error( 'connect.issuer_mismatch', array( 'iss' => $iss ) );
			self::store_notices( array( self::generic_failure_notice() ) );
			return self::settings_url();
		}

		$client = get_option( XPayEG_Constants::OPTION_CONNECT_CLIENT, null );
		if ( ! is_array( $client ) || empty( $client['client_id'] ) || empty( $client['redirect_uri'] ) ) {
			...
			XPayEG_Logger::error( 'connect.client_missing', array() );
			self::store_notices( array( self::generic_failure_notice() ) );
			return self::settings_url();
		}

		try {
			$tokens = XPayEG_Connect_Client::exchange(
			...
			XPayEG_Logger::error( 'connect.exchange_failed', array( 'code' => $e->get_error_code() ) );
			self::store_notices( array( self::generic_failure_notice() ) );
			return self::settings_url();
		}
		...
		if ( null === $keys ) {
			XPayEG_Logger::error( 'connect.response_invalid', array( 'live_mode' => $live ) );
			self::store_notices( array( self::generic_failure_notice() ) );
			return self::settings_url();
		}
		...
		XPayEG_Logger::event( 'connect.completed', array( 'live_mode' => $live ) );
	...
	private static function apply_keys( array $keys, bool $live ): void {
		...
		$option_key = 'woocommerce_' . XPayEG_Constants::GATEWAY_ID . '_settings';
		$settings   = get_option( $option_key, array() );
		update_option( $option_key, XPayEG_Webhook_Configurator::decommission_after_key_update( is_array( $settings ) ? $settings : array() ) );

		$result  = XPayEG_Plugin::instance()->gateway()->validate_and_provision();
	...
	private static function store_notices( array $notices ): void {
		set_transient( 'xpayeg_connect_notice_' . get_current_user_id(), $notices, self::NOTICE_TTL_SECONDS );
	}
	...
	public static function take_notices(): array {
		$key     = 'xpayeg_connect_notice_' . get_current_user_id();
	...
	private static function refusal_notice( string $error, string $description ): array {
		if ( XPayEG_Error_Codes::OAUTH_ACCESS_DENIED === $error ) {
			return array(
				'type' => 'error',
				'text' => '' !== $description
					? sprintf(
	...
	private static function generic_failure_notice(): array {
		return array(
			'type' => 'error',
			'text' => __( 'XPay: the connection could not be completed. Nothing changed. Try again in a moment. If it keeps failing, the WooCommerce logs (source "xpayeg") carry the details.', 'xpay-for-woocommerce' ),
		);
	}
