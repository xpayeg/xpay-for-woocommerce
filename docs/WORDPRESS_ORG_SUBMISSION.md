...
### Codebase state (already done in v2.0.0)

- [x] Plugin name does not start with "WordPress" or "WooCommerce" (now "XPay for WooCommerce")
- [x] Plugin slug matches text domain (`xpay-for-woocommerce`)
- [x] `readme.txt` present at plugin root with all required headers
- [x] `License: GPL-2.0-or-later` declared in plugin header AND in `readme.txt`
- [x] No third-party CDN dependencies (Bootstrap was removed in v1.3.0)
- [x] No tracking/telemetry beyond operational XPay API calls
- [x] External services + Privacy disclosure sections in `readme.txt`
- [x] Bundled `.pot` translation template in `/languages/`
- [x] Plugin Check (PCP) findings reduced to 3 unfixable architecturals
- [x] All PHP files lint clean
- [x] End-to-end verified against XPay staging
...
3. **WordPress.org plugin-page assets (banner + icon + screenshots)** **[YOU]**
   ...
   - **You can submit the plugin ZIP for review BEFORE these are ready** — assets can be added after approval. But the screenshot section in `readme.txt` references 4 screenshots, so you'll want them ready close to launch.
...
```
dist/xpay-for-woocommerce-2.0.0.zip
```

**Verify the ZIP** by opening it locally:

```bash
unzip -l dist/xpay-for-woocommerce-2.0.0.zip | head -30
```

Confirm:
- Top-level directory is `xpay-for-woocommerce/`
- `readme.txt` is present
- `woocommerce-xpay-gateway.php` is present
- `languages/xpay-for-woocommerce.pot` is present
- NO `.git`, `.gitignore`, `.distignore`, `bin/`, `dist/`, `docs/`, `README.md`, `CHANGELOG.md`, `wporg-assets/`
...
- WooCommerce → Settings → Payments shows "Xpay" gateway
- Configuring credentials and placing a test order works end-to-end
- Plugin shows up in WP Admin → Plugins with the right name + version

Use a separate test site or a fresh `wp-env` instance. Do NOT smoke-test on a site that already has the legacy plugin — the slug change means WP treats it as a new plugin (which is correct, but it complicates the test).
...
3. Upload `dist/xpay-for-woocommerce-2.0.0.zip`
4. Confirm:
...
- **Rejected** — you'll get a rationale. Most rejections are about: trademark in plugin name (already addressed), missing License header (already addressed), `Tested up to` too old (currently 6.9, valid for ~6 months), tracking/telemetry without consent (we have none), or third-party CDN dependencies (we have none). If you do get rejected, address the feedback and reply on-thread with a fixed ZIP.

### Common review feedback patterns to expect for THIS plugin

Even with our cleanup, the WP.org reviewers may flag:

- **The two intentional public webhook entry points (`update_order.php`, `check_transaction.php`)** for missing `defined('ABSPATH') or exit;`. These cannot be fixed without breaking the architecture — XPay POSTs directly to `update_order.php` and customer browsers poll `check_transaction.php` from outside any WP request. Reply explaining the design and pointing to the IIFE wrap and `wp-load.php` require at the top of each file.
- **`set_time_limit()`** in `woocommerce-xpay-gateway.php` (already phpcs:ignored, but reviewers may ask). Reply explaining XPay's pay-call worst-case latency exceeds the 30s default and there's no PHP equivalent for extending one request's budget.
- **The `error_log()` calls** in `update_order.php` and `utils.php` (file-level `phpcs:disable` already added). Reply explaining these are always-on diagnostics for webhook signature outcomes / circuit-breaker state, separate from the optional bundled diagnostic logger.
...
## 4. First publish — push v2.0.0 to SVN
...
# 4. Commit
svn ci -m "Release 2.0.0 — initial wordpress.org listing"
```
...
WP.org reads the `Stable tag:` line in `trunk/readme.txt` to decide which `tags/X.Y.Z/` directory to serve. As long as `Stable tag: 2.0.0` matches the `tags/2.0.0/` directory you just created, WP.org will offer 2.0.0 as the install version.
...
## 6. Per-release publishing (every version after v2.0.0)
...
1. `woocommerce-xpay-gateway.php` plugin header `Version:` line
2. `WC_XPAY_VERSION` constant on the next non-comment line
...
| [`woocommerce-xpay-gateway.php`](../woocommerce-xpay-gateway.php) | Plugin main file with the canonical headers (Plugin Name, License, Text Domain, etc.) WP.org indexes. |
| [`languages/xpay-for-woocommerce.pot`](../languages/xpay-for-woocommerce.pot) | Translator template. WP.org's GlotPress imports from here to enable community translations. |
...
# 4. Smoke-test on a fresh WP install (use the ZIP at dist/xpay-for-woocommerce-2.0.0.zip)

# 5. Submit at https://wordpress.org/plugins/developers/add/
#    Upload dist/xpay-for-woocommerce-2.0.0.zip
